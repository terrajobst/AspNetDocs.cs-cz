...
Vytvoří novou úložnou proceduru přidejte do projektu, klikněte pravým tlačítkem na `ManagedDatabaseConstructs` název projektu a zvolte Přidat novou úložnou proceduru. Pojmenujte soubor `GetProductsWithPriceLessThan.cs`. Jak jsme viděli v kroku 3, vytvoří se nový soubor třídy C# s metodu s názvem `GetProductsWithPriceLessThan` umístit `partial` třídy `StoredProcedures`.
