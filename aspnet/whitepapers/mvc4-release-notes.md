...
- **Plná podpora pro trasy:** Rozhraní ASP.NET Web API podporuje kompletní sadu funkcí trasu směrování ASP.NET, včetně parametry trasy a omezení. Kromě toho pomocí jednoduchého konvence mapování akcí na metody HTTP.
- **Vyjednávání obsahu:** Klient a server může společně pracují na určení správný formát pro data se vrací z webového rozhraní API. Rozhraní ASP.NET Web API poskytuje výchozí podporu pro XML, JSON, a kódovaná adresou URL formuláře formátů a mohou rozšířit tuto podporu tak, že přidáte vlastní formátovací moduly, nebo dokonce nahrazujte výchozí strategie vyjednávání obsahu.
...
- **Generování odkazů:** Pomocí rozhraní ASP.NET Web API *UrlHelper* ke generování odkazů k souvisejícím prostředkům ve stejné aplikaci.
...
- **Za konfiguraci kontroleru:** Kontrolery rozhraní ASP.NET Web API mohou být přiděleny s vlastní atribut, který implementuje *IControllerConfiguration* nastavit vlastní formátovací moduly, selektor akcí a parametr vazače. *HttpControllerConfigurationAttribute* byla odebrána.
...
- **MediaRangeMapping odebrány:** Vyjednavač obsahu výchozí nyní zpracovávají rozsahy typu média.
- **Výchozí parametr vazby parametrů jednoduchého typu, je teď [FromUri]:** V předchozích verzích rozhraní ASP.NET Web API používá výchozí parametr vazby parametrů jednoduchého typu vazby modelu. Výchozí parametr vazby parametrů jednoduchého typu, je teď *[FromUri]*.
...
- **Objekt MediaTypeFormatter vylepšení:** Formátovací moduly mají nyní přístup k úplné *HttpContent* instance.
...
- **V hostiteli certifikátům přístup klientské certifikáty:** Použití *GetClientCertificate* metodu rozšíření k získání zadaný certifikát klienta ze zprávy požadavku.
...
- **Pomocník pro zapojení do obslužné rutiny zpráv:** Nové *HttpClientFactory* , která můžete nastavit *DelegatingHandlers* a vytvoření *HttpClient* s požadovanou kanálu, jste připravení začít. Také poskytuje funkce pro její nahoru s alternativní vnitřní obslužné rutiny (výchozí hodnota je *HttpClientHandler*) stejně jako provádět vedení pomocí *HttpMessageInvoker* nebo jiném  *DelegatingHandler* místo *HttpClient* jako původce volání nahoře.
- **Podpora CDN v rozhraní ASP.NET Web optimalizace:** ASP.NET Web optimalizace teď poskytuje podporu pro CDN alternativní cesty umožňuje zadat pro každou sadu další adresa URL který odkazuje na tento stejný prostředek na síť pro doručování obsahu. Podpora CDN umožní, abyste váš skript a styl sady blíž koncovým uživatelům webových aplikací. Produkční aplikace by měla implementovat záložní CDN není k dispozici. Testování na náhradní řešení.
...
- **Změna projektu ASP.NET MVC 4 do cíle 4.0 4.5 nelze aktualizovat odkaz na objektu EntityFramework sestavení:** Pokud změníte projektu ASP.NET MVC 4 do cíle 4.0 po zaměřen 4.5 odkaz na sestavení objektu EntityFramework, se stále odkazují na verze 4.5. Chcete-li vyřešit tento problém odinstalovat a znovu nainstalujte balíček NuGet objektu EntityFramework.
