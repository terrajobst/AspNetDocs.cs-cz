...
[Stažení kódu](https://download.microsoft.com/download/6/0/3/6032582f-360d-4739-b935-38721fdb86ea/CS.10.zip) nebo [stažení PDF](https://download.microsoft.com/download/6/0/3/6032582f-360d-4739-b935-38721fdb86ea/aspnet_tutorial10_AssigningRoles_cs.pdf)
...
[![vlastnost CssClass popisku má za následek velké červené písmo.](assigning-roles-to-users-cs/_static/image2.png)](assigning-roles-to-users-cs/_static/image1.png)

**Obrázek 1**: vlastnost `CssClass` popisku má za následek velké červené písmo ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image3.png)
...
[![stránce se zobrazí uživatelé a role.](assigning-roles-to-users-cs/_static/image5.png)](assigning-roles-to-users-cs/_static/image4.png)

**Obrázek 2**: stránka zobrazuje uživatele a role ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image6.png)
...
> [!NOTE]
> Pokud používáte ASP.NET verze 2,0, nebude zkompilována syntaxe `selectedUserRoles.Contains<string>(...)`. Metoda `Contains<string>` je součástí [knihovny LINQ](http://en.wikipedia.org/wiki/Language_Integrated_Query), která je v ASP.NET 3,5 novinkou. Pokud stále používáte verzi ASP.NET 2,0, použijte místo toho [metodu`Array.IndexOf<string>`](https://msdn.microsoft.com/library/eha9t187.aspx) .
...
[k rolím správců a vedoucích se přidala ![tito.](assigning-roles-to-users-cs/_static/image8.png)](assigning-roles-to-users-cs/_static/image7.png)

**Obrázek 3**: tito bylo přidáno do rolí Správci a vedoucí ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image9.png)
...
[do role Nadřízený byl přidán ![Bruce](assigning-roles-to-users-cs/_static/image11.png)](assigning-roles-to-users-cs/_static/image10.png)

**Obrázek 4**: Bruce bylo přidáno do role Nadřízený ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image12.png)).
...
[![rolí se zobrazují v RoleList DropDownList](assigning-roles-to-users-cs/_static/image14.png)](assigning-roles-to-users-cs/_static/image13.png)

**Obrázek 5**: role se zobrazují v `RoleList` DropDownList ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image15.png)
...
[![v prvku GridView zobrazí seznam uživatelů, kteří patří do vybrané role.](assigning-roles-to-users-cs/_static/image17.png)](assigning-roles-to-users-cs/_static/image16.png)

**Obrázek 6**: v ovládacím prvku GridView zobrazí seznam uživatelů, kteří patří do vybrané role ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image18.png)
...
**Obrázek 7**: Přidání tlačítka "odebrat" do prvku GridView ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image21.png))
...
> [!NOTE]
> Tlačítko Odebrat nevyžaduje před odebráním uživatele z této role žádné potvrzení od uživatele. Beru na schůzku, abyste přidali určitou úroveň potvrzení uživatele. Jedním z nejjednodušších způsobů, jak potvrdit akci, je použít dialogové okno pro potvrzení na straně klienta. Další informace o tomto postupu najdete v tématu [Přidání potvrzení na straně klienta při odstraňování](https://asp.net/learn/data-access/tutorial-42-cs.aspx).

Obrázek 8 ukazuje stránku po odebrání uživatele tito ze skupiny vedoucích.

[![Alas, tito už není správce.](assigning-roles-to-users-cs/_static/image23.png)](assigning-roles-to-users-cs/_static/image22.png)

**Obrázek 8**: Alas, tito již není správce ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image24.png)).
...
[![nemůžete do role přidat neexistujícího uživatele](assigning-roles-to-users-cs/_static/image26.png)](assigning-roles-to-users-cs/_static/image25.png)

**Obrázek 9**: do role nemůžete přidat neexistujícího uživatele ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image27.png)).

Teď zkuste přidat platného uživatele. Pokračujte a znovu přidejte tito do role vedoucí.

[![tito je znovu vedoucí!](assigning-roles-to-users-cs/_static/image29.png)](assigning-roles-to-users-cs/_static/image28.png)

**Obrázek 10**: tito je potom znovu vedoucí!  ([Kliknutím zobrazíte obrázek v plné velikosti.](assigning-roles-to-users-cs/_static/image30.png))
...
Když je účet vytvořen správcem nějaké jiné osoby, může být užitečné, aby správce mohl určit, k jakým rolím nový uživatelský účet patří. <a id="_msoanchor_4"> </a>V kurzu [ *ukládání* *dalších informací o uživatelích* ](../membership/storing-additional-user-information-cs.md) jsme viděli, jak přizpůsobit ovládacím CreateUserWizard přidáním dalších `WizardSteps`. Pojďme se podívat na to, jak do ovládacím CreateUserWizard přidat další krok, abyste mohli zadat role nového uživatele.
...
**Obrázek 11**: přidejte `WizardStep` "zadat role" do ovládacím CreateUserWizard ([kliknutím zobrazíte obrázek v plné velikosti).](assigning-roles-to-users-cs/_static/image33.png)
...
V `WizardStep`zadejte role přidejte CheckBoxList s názvem `RoleList`. Tato funkce CheckBoxList zobrazí seznam dostupných rolí a umožní uživateli navštívit stránku, aby zkontrolovala, k jakým rolím patří nově vytvořený uživatel.
...
Výše uvedený kód by měl vypadat dobře. <a id="_msoanchor_5"> </a>V kurzu [ *ukládání* *dalších informací o uživateli* ](../membership/storing-additional-user-information-cs.md) jsme použili dva příkazy `FindControl` pro odkazování na webový ovládací prvek z vlastního `WizardStep`. A kód, který váže role k CheckBoxList, byl získán z výše v tomto kurzu.
...
[![vytvořit nového uživatele s názvem Wanda](assigning-roles-to-users-cs/_static/image35.png)](assigning-roles-to-users-cs/_static/image34.png)

**Obrázek 12**: vytvoření nového uživatele s názvem Wanda ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image36.png))
...
[![Wanda členství v roli správce](assigning-roles-to-users-cs/_static/image38.png)](assigning-roles-to-users-cs/_static/image37.png)

**Obrázek 13**: vytvoření Wanda člena role Nadřízený ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image39.png))
...
[Všechny správce ![Bruce, tito a Wanda](assigning-roles-to-users-cs/_static/image41.png)](assigning-roles-to-users-cs/_static/image40.png)

**Obrázek 14**: Bruce, tito a Wanda jsou všichni vedoucí ([kliknutím zobrazíte obrázek v plné velikosti](assigning-roles-to-users-cs/_static/image42.png)).
...
V tuto chvíli jsme viděli, jak vytvořit a odstranit role a jak přidávat a odebírat uživatele z rolí. Ale zatím jsme se vyhledali při použití autorizace na základě rolí. <a id="_msoanchor_6"> </a>V [následujícím kurzu](role-based-authorization-cs.md) se podíváme na definování autorizačních pravidel URL pro role na základě rolí a také omezení funkcí na úrovni stránek na základě rolí aktuálně přihlášeného uživatele.
