...
2. <span data-ttu-id="bfe11-185">Zkopírujte následující kód a označení na stránku a nahraďte cokoli již na stránce.</span><span class="sxs-lookup"><span data-stu-id="bfe11-185">Copy the following code and markup into the page, replacing anything already in the page.</span></span>  
    ...
    <span data-ttu-id="bfe11-186">Tady je několik věcí, které můžete poznamenat:</span><span class="sxs-lookup"><span data-stu-id="bfe11-186">Here are some things for you to note:</span></span>
...
<span data-ttu-id="bfe11-314">*Typ String nejde implicitně převést na int.*</span><span class="sxs-lookup"><span data-stu-id="bfe11-314">*Cannot implicitly convert type 'string' to 'int'.*</span></span>
...
## <a name="operators"></a><span data-ttu-id="bfe11-328">Operátory</span><span class="sxs-lookup"><span data-stu-id="bfe11-328">Operators</span></span>
...
<span data-ttu-id="bfe11-410">V tomto příkladu, pokud není první podmínka v bloku If nastavena na hodnotu true, je zaškrtnuta podmínka `else if`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="bfe11-411">Pokud je tato podmínka splněna, jsou provedeny příkazy v bloku `else if`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="bfe11-412">Pokud není splněna žádná z podmínek, jsou provedeny příkazy v bloku `else`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="bfe11-413">Můžete přidat libovolný počet else if Blocks a pak zavřít s `else` blok jako &quot;vše jiného&quot; podmínka.</span><span class="sxs-lookup"><span data-stu-id="bfe11-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="bfe11-414">K otestování velkého počtu podmínek použijte `switch` blok:</span><span class="sxs-lookup"><span data-stu-id="bfe11-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="bfe11-415">Hodnota, která se má testovat, je v závorkách (v příkladu `weekday` proměnné).</span><span class="sxs-lookup"><span data-stu-id="bfe11-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="bfe11-416">Každý jednotlivý test používá příkaz `case`, který končí dvojtečkou (:).</span><span class="sxs-lookup"><span data-stu-id="bfe11-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="bfe11-417">Je-li hodnota příkazu `case` shodná s testovací hodnotou, je proveden kód v tomto bloku případu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="bfe11-418">Všechny příkazy Case zavřete příkazem `break`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="bfe11-419">(Pokud zapomenete zahrnout do každého bloku `case`, kód z dalšího příkazu `case` se spustí také.) `switch` blok často obsahuje `default` příkaz jako poslední případ pro &quot;vše ostatní&quot; možnost, která se spustí, pokud žádný z ostatních případů neplatí.</span><span class="sxs-lookup"><span data-stu-id="bfe11-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="bfe11-420">Výsledek posledního dvou podmíněných bloků zobrazených v prohlížeči:</span><span class="sxs-lookup"><span data-stu-id="bfe11-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor – Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="bfe11-422">Kód smyčky</span><span class="sxs-lookup"><span data-stu-id="bfe11-422">Looping Code</span></span>

<span data-ttu-id="bfe11-423">Často je třeba spouštět stejné příkazy opakovaně.</span><span class="sxs-lookup"><span data-stu-id="bfe11-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="bfe11-424">Provedete to pomocí smyčky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-424">You do this by looping.</span></span> <span data-ttu-id="bfe11-425">Například často spustíte stejné příkazy pro každou položku v kolekci dat.</span><span class="sxs-lookup"><span data-stu-id="bfe11-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="bfe11-426">Pokud víte přesně, kolikrát chcete cyklovat, můžete použít smyčku `for`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="bfe11-427">Tento druh smyčky je zvláště užitečný pro inventarizaci a počítání:</span><span class="sxs-lookup"><span data-stu-id="bfe11-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="bfe11-428">Smyčka začíná klíčovým slovem `for`, po kterém následují tři příkazy v závorkách, přičemž každý z nich končí středníkem.</span><span class="sxs-lookup"><span data-stu-id="bfe11-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="bfe11-429">V závorkách první příkaz (`var i=10;`) vytvoří čítač a inicializuje jej na hodnotu 10.</span><span class="sxs-lookup"><span data-stu-id="bfe11-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="bfe11-430">Nemusíte pojmenovat `i` &#8212; čítače. můžete použít libovolnou proměnnou.</span><span class="sxs-lookup"><span data-stu-id="bfe11-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="bfe11-431">Po spuštění smyčky `for` se čítač automaticky zvýší.</span><span class="sxs-lookup"><span data-stu-id="bfe11-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="bfe11-432">Druhý příkaz (`i < 21;`) nastaví podmínku pro jak daleko chcete počítat.</span><span class="sxs-lookup"><span data-stu-id="bfe11-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="bfe11-433">V takovém případě chcete, aby bylo možné přejít maximálně o 20 (to znamená, že bude pokračovat i v případě, že je čítač menší než 21).</span><span class="sxs-lookup"><span data-stu-id="bfe11-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="bfe11-434">Třetí příkaz (`i++`) používá operátor přírůstek, který jednoduše určuje, že čítač by měl mít 1 přidaný čítač při každém spuštění smyčky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="bfe11-435">Uvnitř složených závorek je kód, který se spustí pro každou iteraci smyčky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="bfe11-436">Kód vytvoří nový odstavec (`<p>` element) pokaždé a přidá řádek do výstupu a zobrazí hodnotu `i` (čítač).</span><span class="sxs-lookup"><span data-stu-id="bfe11-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="bfe11-437">Když spustíte tuto stránku, v příkladu se vytvoří 11 řádků, ve kterých se zobrazí výstup, a text na každém řádku, který označuje číslo položky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor – img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="bfe11-439">Pokud pracujete s kolekcí nebo polem, často používáte smyčku `foreach`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="bfe11-440">Kolekce je skupina podobných objektů a smyčka `foreach` umožňuje provádět úlohy na každé položce v kolekci.</span><span class="sxs-lookup"><span data-stu-id="bfe11-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="bfe11-441">Tento typ smyčky je vhodný pro kolekce, protože na rozdíl od `for` smyčky nemusíte zvyšovat čítač nebo nastavit limit.</span><span class="sxs-lookup"><span data-stu-id="bfe11-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="bfe11-442">Místo toho kód smyčky `foreach` jednoduše projde přes kolekci, dokud není dokončena.</span><span class="sxs-lookup"><span data-stu-id="bfe11-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="bfe11-443">Například následující kód vrátí položky v kolekci `Request.ServerVariables`, což je objekt, který obsahuje informace o vašem webovém serveru.</span><span class="sxs-lookup"><span data-stu-id="bfe11-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="bfe11-444">Používá smyčku `foreac` h k zobrazení názvu každé položky vytvořením nového prvku `<li>` v seznamu s odrážkami HTML.</span><span class="sxs-lookup"><span data-stu-id="bfe11-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="bfe11-445">Za klíčovým slovem `foreach` následuje závorky, kde deklarujete proměnnou reprezentující jednu položku v kolekci (v příkladu `var item`) následovaný klíčovým slovem `in` následovaným kolekcí, kterou chcete procyklovat.</span><span class="sxs-lookup"><span data-stu-id="bfe11-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="bfe11-446">V těle `foreach` smyčky můžete k aktuální položce přistupovat pomocí proměnné, kterou jste předtím deklarovali.</span><span class="sxs-lookup"><span data-stu-id="bfe11-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor – Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="bfe11-448">Chcete-li vytvořit obecnější smyčku pro účely, použijte příkaz `while`:</span><span class="sxs-lookup"><span data-stu-id="bfe11-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="bfe11-449">Smyčka `while` začíná klíčovým slovem `while` následovanými závorkami, kde určíte, jak dlouho bude smyčka pokračovat (zde, pokud je `countNum` menší než 50), pak se blok opakuje.</span><span class="sxs-lookup"><span data-stu-id="bfe11-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="bfe11-450">Smyčky typicky přidávají (přidávají do) nebo snižují (odečtou z) proměnnou nebo objekt, který se používá pro počítání.</span><span class="sxs-lookup"><span data-stu-id="bfe11-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="bfe11-451">V příkladu operátor `+=` přidá 1 pro `countNum` pokaždé, když se smyčka spustí.</span><span class="sxs-lookup"><span data-stu-id="bfe11-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="bfe11-452">(Chcete-li snížit proměnnou ve smyčce, která počítá dolů, použijte operátor snížení `-=`).</span><span class="sxs-lookup"><span data-stu-id="bfe11-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="bfe11-453">Objekty a kolekce</span><span class="sxs-lookup"><span data-stu-id="bfe11-453">Objects and Collections</span></span>

<span data-ttu-id="bfe11-454">Skoro všechno na webu ASP.NET je objekt, včetně samotné webové stránky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="bfe11-455">Tato část popisuje některé důležité objekty, se kterými se často pracujete ve vašem kódu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="bfe11-456">Objekty stránky</span><span class="sxs-lookup"><span data-stu-id="bfe11-456">Page Objects</span></span>

<span data-ttu-id="bfe11-457">Nejzákladnější objekt v ASP.NET je stránka.</span><span class="sxs-lookup"><span data-stu-id="bfe11-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="bfe11-458">K vlastnostem objektu stránky můžete přistupovat přímo bez jakéhokoli opravňujícího objektu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="bfe11-459">Následující kód Získá cestu k souboru stránky pomocí objektu `Request` stránky:</span><span class="sxs-lookup"><span data-stu-id="bfe11-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="bfe11-460">Chcete-li zrušit zaškrtnutí, že odkazujete na vlastnosti a metody v objektu aktuální stránky, můžete volitelně použít klíčové slovo `this` pro reprezentaci objektu Page v kódu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="bfe11-461">Zde je předchozí příklad kódu s `this` přidat k reprezentaci stránky:</span><span class="sxs-lookup"><span data-stu-id="bfe11-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="bfe11-462">Pomocí vlastností objektu `Page` můžete získat velké množství informací, například:</span><span class="sxs-lookup"><span data-stu-id="bfe11-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="bfe11-463">`Request`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-463">`Request`.</span></span> <span data-ttu-id="bfe11-464">Jak už jste viděli, jedná se o kolekci informací o aktuální žádosti, včetně typu prohlížeče, který požadavek odeslal, adresu URL stránky, identitu uživatele atd.</span><span class="sxs-lookup"><span data-stu-id="bfe11-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="bfe11-465">`Response`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-465">`Response`.</span></span> <span data-ttu-id="bfe11-466">Toto je kolekce informací o odpovědi (stránky), která se odešle do prohlížeče, když je kód serveru spuštěný.</span><span class="sxs-lookup"><span data-stu-id="bfe11-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="bfe11-467">Tuto vlastnost můžete například použít k zápisu informací do odpovědi.</span><span class="sxs-lookup"><span data-stu-id="bfe11-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="bfe11-468">Objekty kolekce (pole a slovníky)</span><span class="sxs-lookup"><span data-stu-id="bfe11-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="bfe11-469">*Kolekce* je skupina objektů stejného typu, například kolekce `Customer` objektů z databáze.</span><span class="sxs-lookup"><span data-stu-id="bfe11-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="bfe11-470">ASP.NET obsahuje mnoho předdefinovaných kolekcí, jako je kolekce `Request.Files`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="bfe11-471">Často budete pracovat s daty v kolekcích.</span><span class="sxs-lookup"><span data-stu-id="bfe11-471">You'll often work with data in collections.</span></span> <span data-ttu-id="bfe11-472">Dva společné typy kolekcí jsou *pole* a *slovník*.</span><span class="sxs-lookup"><span data-stu-id="bfe11-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="bfe11-473">Pole je užitečné, pokud chcete uložit kolekci podobných položek, ale nechcete vytvořit samostatnou proměnnou pro uchování každé položky:</span><span class="sxs-lookup"><span data-stu-id="bfe11-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="bfe11-474">Pomocí polí deklarujete konkrétní datový typ, například `string`, `int`nebo `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="bfe11-475">Chcete-li označit, že proměnná může obsahovat pole, přidejte do deklarace hranaté závorky (například `string[]` nebo `int[]`).</span><span class="sxs-lookup"><span data-stu-id="bfe11-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="bfe11-476">K položkám v poli můžete přistupovat pomocí jejich pozice (indexu) nebo pomocí příkazu `foreach`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="bfe11-477">Indexy polí jsou počítány od &#8212; nuly, tj. první položka je na pozici 0, druhá položka je na pozici 1 atd.</span><span class="sxs-lookup"><span data-stu-id="bfe11-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="bfe11-478">Počet položek v poli můžete určit získáním jeho vlastnosti `Length`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="bfe11-479">Chcete-li získat pozici konkrétní položky v poli (pro hledání v poli), použijte metodu `Array.IndexOf`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="bfe11-480">Můžete také provést akce, jako je vrácení obsahu pole (`Array.Reverse` metody) nebo řazení obsahu (metoda `Array.Sort`).</span><span class="sxs-lookup"><span data-stu-id="bfe11-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="bfe11-481">Výstup kódu řetězcového pole zobrazeného v prohlížeči:</span><span class="sxs-lookup"><span data-stu-id="bfe11-481">The output of the string array code displayed in a browser:</span></span>

![Razor – Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="bfe11-483">Slovník je kolekce párů klíč/hodnota, kde zadáte klíč (nebo název) pro nastavení nebo načtení příslušné hodnoty:</span><span class="sxs-lookup"><span data-stu-id="bfe11-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="bfe11-484">Chcete-li vytvořit slovník, použijte klíčové slovo `new` pro indikaci, že vytváříte nový objekt Dictionary.</span><span class="sxs-lookup"><span data-stu-id="bfe11-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="bfe11-485">K proměnné můžete přiřadit slovník pomocí klíčového slova `var`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="bfe11-486">Datové typy položek ve slovníku označíte pomocí lomených závorek (`< >`).</span><span class="sxs-lookup"><span data-stu-id="bfe11-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="bfe11-487">Na konci deklarace je nutné přidat dvojici závorek, protože se jedná o metodu, která vytvoří nový slovník.</span><span class="sxs-lookup"><span data-stu-id="bfe11-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="bfe11-488">Chcete-li přidat položky do slovníku, můžete zavolat metodu `Add` proměnné Dictionary (`myScores` v tomto případě) a poté zadat klíč a hodnotu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="bfe11-489">Alternativně můžete pomocí hranatých závorek označit klíč a provést jednoduché přiřazení, jako v následujícím příkladu:</span><span class="sxs-lookup"><span data-stu-id="bfe11-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="bfe11-490">Pokud chcete získat hodnotu ze slovníku, zadáte klíč v závorkách:</span><span class="sxs-lookup"><span data-stu-id="bfe11-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="bfe11-491">Volání metod s parametry</span><span class="sxs-lookup"><span data-stu-id="bfe11-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="bfe11-492">Jak si přečtete výše v tomto článku, objekty, se kterými program pracuje, můžou mít metody.</span><span class="sxs-lookup"><span data-stu-id="bfe11-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="bfe11-493">Například `Database` objekt může mít metodu `Database.Connect`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="bfe11-494">Mnoho metod má také jeden nebo více parametrů.</span><span class="sxs-lookup"><span data-stu-id="bfe11-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="bfe11-495">*Parametr* je hodnota, kterou předáte metodě, aby mohla metoda dokončit její úlohu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="bfe11-496">Podívejte se například na deklaraci metody `Request.MapPath`, která přijímá tři parametry:</span><span class="sxs-lookup"><span data-stu-id="bfe11-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="bfe11-497">(Řádek byl zabalen, aby byl čitelnější.</span><span class="sxs-lookup"><span data-stu-id="bfe11-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="bfe11-498">Mějte na paměti, že můžete umístit konce řádků téměř na jakékoli místo kromě řetězců, které jsou uzavřeny v uvozovkách.)</span><span class="sxs-lookup"><span data-stu-id="bfe11-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="bfe11-499">Tato metoda vrátí fyzickou cestu na serveru, který odpovídá zadané virtuální cestě.</span><span class="sxs-lookup"><span data-stu-id="bfe11-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="bfe11-500">Tři parametry pro metodu jsou `virtualPath`, `baseVirtualDir`a `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="bfe11-501">(Všimněte si, že v deklaraci jsou parametry uvedeny s datovými typy dat, která budou přijímat.) Při volání této metody je nutné dodat hodnoty pro všechny tři parametry.</span><span class="sxs-lookup"><span data-stu-id="bfe11-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="bfe11-502">Syntaxe Razor poskytuje dvě možnosti pro předávání parametrů metodě: *poziční parametry* a *pojmenované parametry*.</span><span class="sxs-lookup"><span data-stu-id="bfe11-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="bfe11-503">Chcete-li volat metodu pomocí pozičních parametrů, předáte parametry v přesném pořadí, které je zadáno v deklaraci metody.</span><span class="sxs-lookup"><span data-stu-id="bfe11-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="bfe11-504">(Toto pořadí byste obvykle znali v dokumentaci k metodě.) Je nutné postupovat podle pořadí a v případě potřeby nemůžete přeskočit &#8212; žádný z parametrů, předáte prázdný řetězec (`""`) nebo `null` pozičního parametru, pro který nemáte hodnotu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="bfe11-505">Následující příklad předpokládá, že máte ve svém webu složku s názvem *skripty* .</span><span class="sxs-lookup"><span data-stu-id="bfe11-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="bfe11-506">Kód volá metodu `Request.MapPath` a předá hodnoty pro tři parametry ve správném pořadí.</span><span class="sxs-lookup"><span data-stu-id="bfe11-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="bfe11-507">Pak zobrazí výslednou mapovanou cestu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="bfe11-508">Pokud má metoda mnoho parametrů, můžete si kód lépe přečíst pomocí pojmenovaných parametrů.</span><span class="sxs-lookup"><span data-stu-id="bfe11-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="bfe11-509">Chcete-li volat metodu pomocí pojmenovaných parametrů, zadejte název parametru následovaný dvojtečkou (:) a pak hodnotu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="bfe11-510">Výhodou pojmenovaných parametrů je, že je můžete předat v libovolném požadovaném pořadí.</span><span class="sxs-lookup"><span data-stu-id="bfe11-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="bfe11-511">(Nevýhodou je, že volání metody není jako kompaktní.)</span><span class="sxs-lookup"><span data-stu-id="bfe11-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="bfe11-512">Následující příklad volá stejnou metodu jako výše, ale používá pojmenované parametry k poskytnutí těchto hodnot:</span><span class="sxs-lookup"><span data-stu-id="bfe11-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="bfe11-513">Jak vidíte, parametry jsou předány v jiném pořadí.</span><span class="sxs-lookup"><span data-stu-id="bfe11-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="bfe11-514">Pokud však spustíte předchozí příklad a tento příklad vrátí stejnou hodnotu.</span><span class="sxs-lookup"><span data-stu-id="bfe11-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="bfe11-515">Zpracování chyb</span><span class="sxs-lookup"><span data-stu-id="bfe11-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="bfe11-516">Příkazy try-catch</span><span class="sxs-lookup"><span data-stu-id="bfe11-516">Try-Catch Statements</span></span>

<span data-ttu-id="bfe11-517">Často budete mít ve svém kódu příkazy, které mohou selhat z důvodů mimo váš ovládací prvek.</span><span class="sxs-lookup"><span data-stu-id="bfe11-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="bfe11-518">Příklad:</span><span class="sxs-lookup"><span data-stu-id="bfe11-518">For example:</span></span>

- <span data-ttu-id="bfe11-519">Pokud se váš kód pokusí vytvořit soubor nebo k němu získat přístup, může dojít k nejrůznějším chybám.</span><span class="sxs-lookup"><span data-stu-id="bfe11-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="bfe11-520">Soubor, který chcete, možná neexistuje, může být uzamčen, kód pravděpodobně nemá oprávnění atd.</span><span class="sxs-lookup"><span data-stu-id="bfe11-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="bfe11-521">Podobně platí, že pokud se váš kód pokusí aktualizovat záznamy v databázi, může dojít k problémům s oprávněními, připojení k databázi může být vyřazeno, data, která mají být uložena, mohou být neplatná atd.</span><span class="sxs-lookup"><span data-stu-id="bfe11-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="bfe11-522">V programovacích podmínkách se tyto situace nazývají *výjimky*.</span><span class="sxs-lookup"><span data-stu-id="bfe11-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="bfe11-523">Pokud váš kód narazí na výjimku, vygeneruje (vyvolá) chybovou zprávu, která je na nejvyšší úrovni uživatelům obtěžující:</span><span class="sxs-lookup"><span data-stu-id="bfe11-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor – Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="bfe11-525">V situacích, kdy se váš kód může setkat s výjimkami, a aby se předešlo chybovým zprávám tohoto typu, můžete použít příkazy `try/catch`.</span><span class="sxs-lookup"><span data-stu-id="bfe11-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="bfe11-526">V příkazu `try` spustíte kód, který kontrolujete.</span><span class="sxs-lookup"><span data-stu-id="bfe11-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="bfe11-527">V jednom nebo více příkazech `catch` můžete vyhledat konkrétní chyby (konkrétní typy výjimek), ke kterým mohlo dojít.</span><span class="sxs-lookup"><span data-stu-id="bfe11-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="bfe11-528">Můžete zahrnout tolik příkazů `catch`, kolik potřebujete pro hledání chyb, které očekáváte.</span><span class="sxs-lookup"><span data-stu-id="bfe11-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="bfe11-529">Doporučujeme, abyste se vyhnuli použití metody `Response.Redirect` v příkazech `try/catch`, protože to může způsobit výjimku na stránce.</span><span class="sxs-lookup"><span data-stu-id="bfe11-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="bfe11-530">Následující příklad ukazuje stránku, která vytvoří textový soubor pro první požadavek a pak zobrazí tlačítko, které uživateli otevře soubor.</span><span class="sxs-lookup"><span data-stu-id="bfe11-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="bfe11-531">Příklad záměrně používá špatný název souboru, takže bude způsobovat výjimku.</span><span class="sxs-lookup"><span data-stu-id="bfe11-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="bfe11-532">Kód zahrnuje `catch` příkazy pro dvě možné výjimky: `FileNotFoundException`, ke kterým dojde, pokud je název souboru špatný a `DirectoryNotFoundException`, ke kterému dochází, pokud ASP.NET nemůže dokonce najít složku.</span><span class="sxs-lookup"><span data-stu-id="bfe11-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="bfe11-533">(Můžete zrušit komentář k příkazu v příkladu, abyste viděli, jak se spustí, když vše funguje správně.)</span><span class="sxs-lookup"><span data-stu-id="bfe11-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="bfe11-534">Pokud váš kód nezpracovává výjimku, zobrazila se chybová stránka, například snímek předchozí obrazovky.</span><span class="sxs-lookup"><span data-stu-id="bfe11-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="bfe11-535">Nicméně část `try/catch` pomáhá zabránit uživateli v zobrazování těchto typů chyb.</span><span class="sxs-lookup"><span data-stu-id="bfe11-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="bfe11-536">Další materiály a zdroje informací</span><span class="sxs-lookup"><span data-stu-id="bfe11-536">Additional Resources</span></span>

<span data-ttu-id="bfe11-537">**Programování pomocí Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="bfe11-537">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="bfe11-538">Příloha: Visual Basic jazyk a syntax</span><span class="sxs-lookup"><span data-stu-id="bfe11-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="bfe11-539">**Referenční dokumentace**</span><span class="sxs-lookup"><span data-stu-id="bfe11-539">**Reference Documentation**</span></span>

[<span data-ttu-id="bfe11-540">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="bfe11-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="bfe11-541">C#Language</span><span class="sxs-lookup"><span data-stu-id="bfe11-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
