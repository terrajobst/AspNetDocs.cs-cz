...
title: 'Správa životního cyklu aplikací: Od vývoje k ostrému provozu | Dokumentace Microsoftu'
author: jrjlee
...
- **Požadavky**: Jak budete muset nakonfigurovat infrastrukturu serveru předtím, než začleníte logiky nasazení na místě.
- **Počáteční vývoj a nasazení**: Co je potřeba udělat před nasazením řešení poprvé.
- **Nasazení testování**: Jak balení a nasazení obsahu do testovacího prostředí automaticky při vývojář vrátí nový kód.
...
- **Nasazení do produkčního prostředí**: Jak importovat webových balíčků do produkčního prostředí při vzdálené nasazení brání síťové infrastruktury.
...
- Testovací prostředí je nakonfigurované tak, aby přijímal vzdálené nasazení s využitím služby agenta nasazení webu ("vzdálený agent"), jak je popsáno v [scénář: Konfigurace testovacího prostředí pro nasazení webu](../configuring-server-environments-for-web-deployment/scenario-configuring-a-test-environment-for-web-deployment.md) a [nakonfigurovat webový Server pro nasazení webu (vzdálený Agent) publikování](../configuring-server-environments-for-web-deployment/configuring-a-web-server-for-web-deploy-publishing-remote-agent.md).
- Pracovní prostředí je nakonfigurované tak, aby přijímal vzdálené nasazení s využitím koncového bodu obslužné rutiny nasazení webu, jak je popsáno v [scénář: Konfigurace přípravného prostředí pro nasazení webu](../configuring-server-environments-for-web-deployment/scenario-configuring-a-staging-environment-for-web-deployment.md) a [konfiguraci webového serveru pro publikování nasazeného webu (Obslužná rutina nasazení webu)](../configuring-server-environments-for-web-deployment/configuring-a-web-server-for-web-deploy-publishing-web-deploy-handler.md).
