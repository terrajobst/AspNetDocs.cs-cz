...
>
> V článku se předpokládá, že už máte nějaké znalosti událostí pro signalizaci a životnost připojení. Úvod do signalizace najdete v tématu [Úvod do nástroje Signal](../getting-started/introduction-to-signalr.md). Seznam událostí životního cyklu připojení najdete v následujících zdrojích informací:
>
> - [Postup zpracování událostí životního cyklu připojení ve třídě centra](hubs-api-guide-server.md#connectionlifetime)
> - [Postup zpracování událostí životního cyklu připojení v klientech JavaScript](hubs-api-guide-javascript-client.md#connectionlifetime)
> - [Postup zpracování událostí životního cyklu připojení v klientech rozhraní .NET](hubs-api-guide-net-client.md#connectionlifetime)
>
...
> [!NOTE]
>
> **Důležité**: posloupnost událostí popsaných tady není zaručená. Signalní je pokaždé, když se každý pokus o vyzvednutí událostí životního cyklu připojení předvídatelným způsobem podle tohoto schématu, ale existuje mnoho variant síťových událostí a mnoha způsobů, kterými se podřídí komunikační architektury, jako jsou třeba transportní rozhraní API. Například událost `Reconnected` nemusí být vyvolána, když se klient znovu připojí, nebo se může spustit obslužná rutina `OnConnected` na serveru, když se pokus o navázání spojení nezdařil. V tomto tématu jsou popsány pouze efekty, které by obvykle byly vyprodukovány pomocí určitých typických okolností.
...
Návěstí 2,1 přidá přetížení na server `OnDisconnect` událost, která označuje, jestli se klient úmyslně odpojil, ale nečeká na vypršení časového limitu. Parametr `StopCalled` má hodnotu true, pokud klient explicitně uzavřel připojení. Pokud v jazyce JavaScript chyba serveru vedla klienta k odpojení, budou informace o chybě předány klientovi jako `$.connection.hub.lastError`.

**C#Server kód: `stopCalled` parametr**

[!code-csharp[Main](handling-connection-lifetime-events/samples/sample7.cs?highlight=1,3)]

**JavaScriptový kód klienta: přístup k `lastError` v události `disconnect`.**
