...
Ve výchozím nastavení platí, že při použití atributu [OutputCache] je obsah uložen do mezipaměti ve třech umístěních: webový server, všechny proxy servery a webový prohlížeč. Úpravou vlastnosti Location atributu [OutputCache] můžete ovládat přesně, kde je obsah uložen do mezipaměti.
...
Ukládání výstupu do mezipaměti poskytuje velmi snadný způsob, jak významně zlepšit výkon aplikací ASP.NET MVC. V tomto kurzu jste zjistili, jak pomocí atributu [OutputCache] ukládat výstup akcí kontroleru. Zjistili jste také, jak upravit vlastnosti atributu [OutputCache], jako jsou vlastnosti Duration a VaryByParam, aby bylo možné změnit způsob ukládání obsahu do mezipaměti. Nakonec jste zjistili, jak definovat profily mezipaměti v konfiguračním souboru webu.
