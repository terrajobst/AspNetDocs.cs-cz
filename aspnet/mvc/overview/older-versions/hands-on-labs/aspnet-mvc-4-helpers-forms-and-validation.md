...
1. [<span data-ttu-id="8b89e-139">Vytvoření kontroleru Store Manager a jeho zobrazení indexu</span><span class="sxs-lookup"><span data-stu-id="8b89e-139">Creating the Store Manager controller and its Index view</span></span>](#Exercise1)
...
<span data-ttu-id="8b89e-152">V této úloze vytvoříte nový kontroler volá **StoreManagerController** pro podporu operací CRUD.</span><span class="sxs-lookup"><span data-stu-id="8b89e-152">In this task, you will create a new controller called **StoreManagerController** to support CRUD operations.</span></span>
...
2. <span data-ttu-id="8b89e-161">Přidáte nový kontroler.</span><span class="sxs-lookup"><span data-stu-id="8b89e-161">Add a new controller.</span></span> <span data-ttu-id="8b89e-162">Chcete-li to provést, klikněte pravým tlačítkem **řadiče** složku v Průzkumníku řešení vyberte **přidat** a pak **řadič** příkazu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-162">To do this, right-click the **Controllers** folder within the Solution Explorer, select **Add** and then the **Controller** command.</span></span> <span data-ttu-id="8b89e-163">Změnit **řadič** **název** k **StoreManagerController** a ujistěte se, že možnost **kontroler MVC s akcemi čtení/zápisu-prázdný**zaškrtnuto.</span><span class="sxs-lookup"><span data-stu-id="8b89e-163">Change the **Controller** **Name** to **StoreManagerController** and make sure the option **MVC controller with empty read/write actions** is selected.</span></span> <span data-ttu-id="8b89e-164">Klikněte na **Přidat**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-164">Click **Add**.</span></span>
    ...
    <span data-ttu-id="8b89e-166">*Přidat Dialog Kontroleru*</span><span class="sxs-lookup"><span data-stu-id="8b89e-166">*Add Controller Dialog*</span></span>
...
<span data-ttu-id="8b89e-215">*Procházení seznamu alb s nikoli zkráceno text*</span><span class="sxs-lookup"><span data-stu-id="8b89e-215">*Browsing the list of Albums with not truncated text*</span></span>
...
2. <span data-ttu-id="8b89e-230">Otevřete zobrazení indexu StoreManager společnosti.</span><span class="sxs-lookup"><span data-stu-id="8b89e-230">Open StoreManager's Index View.</span></span> <span data-ttu-id="8b89e-231">Chcete-li to provést, v Průzkumníku řešení rozbalte **zobrazení** složku, pak bude **StoreManager** a otevřete **Index.cshtml** souboru.</span><span class="sxs-lookup"><span data-stu-id="8b89e-231">To do this, in the Solution Explorer expand the **Views** folder, then the **StoreManager** and open the **Index.cshtml** file.</span></span>
...
2. <span data-ttu-id="8b89e-242">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-242">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-243">Změňte adresu URL na **/StoreManager** k ověření tohoto dlouho texty v **Title** a **interpreta** sloupce se zkrátí.</span><span class="sxs-lookup"><span data-stu-id="8b89e-243">Change the URL to **/StoreManager** to verify that long texts in the **Title** and **Artist** column are truncated.</span></span>
    ...
    <span data-ttu-id="8b89e-244">![Oříznutá názvy a umělci názvy](aspnet-mvc-4-helpers-forms-and-validation/_static/image6.png "zkrácen názvy a umělci názvy")</span><span class="sxs-lookup"><span data-stu-id="8b89e-244">![Truncated titles and artists names](aspnet-mvc-4-helpers-forms-and-validation/_static/image6.png "Truncated titles and artists names")</span></span>
...
<span data-ttu-id="8b89e-253">V této úloze budete implementovat verze HTTP GET metody akce úpravy k načtení příslušné alba z databáze, a také seznam všech žánry a umělci.</span><span class="sxs-lookup"><span data-stu-id="8b89e-253">In this task, you will implement the HTTP-GET version of the Edit action method to retrieve the appropriate Album from the database, as well as a list of all Genres and Artists.</span></span> <span data-ttu-id="8b89e-254">To se zabalit tato data do **StoreManagerViewModel** definované v posledním kroku, který se potom předá do šablony zobrazení k vykreslení odpovědi s objekty.</span><span class="sxs-lookup"><span data-stu-id="8b89e-254">It will package this data up into the **StoreManagerViewModel** object defined in the last step, which will then be passed to a View template to render the response with.</span></span>
...
2. <span data-ttu-id="8b89e-275">V dialogovém okně Přidat zobrazení ověřte, zda je název zobrazení **upravit**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-275">In the Add View dialog, verify that the View Name is **Edit**.</span></span> <span data-ttu-id="8b89e-276">Zkontrolujte **vytvoření zobrazení se silnými typy** zaškrtávací políčko a vyberte **alba (MvcMusicStore.Models)** z **zobrazení dat třídy** rozevíracího seznamu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-276">Check the **Create a strongly-typed view** checkbox and select **Album (MvcMusicStore.Models)** from the **View data class** drop-down.</span></span> <span data-ttu-id="8b89e-277">Vyberte **upravit** z **šablona Scaffold** rozevíracího seznamu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-277">Select **Edit** from the **Scaffold template** drop-down.</span></span> <span data-ttu-id="8b89e-278">Ostatní pole nechte výchozí hodnoty a pak klikněte na tlačítko **přidat**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-278">Leave the other fields with their default value and then click **Add**.</span></span>
    ...
    <span data-ttu-id="8b89e-280">*Přidání zobrazení pro úpravy*</span><span class="sxs-lookup"><span data-stu-id="8b89e-280">*Adding an Edit view*</span></span>
...
1. <span data-ttu-id="8b89e-290">Nahradit vše **alba** fieldset kód následujícím kódem:</span><span class="sxs-lookup"><span data-stu-id="8b89e-290">Replace all the **Album** fieldset code with the following:</span></span>
    ...
    > 2. <span data-ttu-id="8b89e-294">**SelectList** hodnot pro rozevírací seznam.</span><span class="sxs-lookup"><span data-stu-id="8b89e-294">The **SelectList** of values for the drop-down.</span></span>
...
2. <span data-ttu-id="8b89e-320">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-320">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-321">Změňte adresu URL na **/StoreManager/Edit/1**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-321">Change the URL to **/StoreManager/Edit/1**.</span></span> <span data-ttu-id="8b89e-322">Změnit název alba **zatížení** a klikněte na **Uložit**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-322">Change the Album title to **Load** and click on **Save**.</span></span> <span data-ttu-id="8b89e-323">Ověřte, že ve skutečnosti změnit název společnosti v seznamu alb.</span><span class="sxs-lookup"><span data-stu-id="8b89e-323">Verify that album's title actually changed in the list of albums.</span></span>
    ...
    <span data-ttu-id="8b89e-324">![Aktualizuje se alba](aspnet-mvc-4-helpers-forms-and-validation/_static/image10.png "aktualizace alba")</span><span class="sxs-lookup"><span data-stu-id="8b89e-324">![Updating an album](aspnet-mvc-4-helpers-forms-and-validation/_static/image10.png "Updating an album")</span></span>
...
1. <span data-ttu-id="8b89e-348">Klepněte pravým tlačítkem myši **vytvořit** metody akce a vyberte **přidat zobrazení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-348">Right-click inside the **Create** action method and select **Add View**.</span></span> <span data-ttu-id="8b89e-349">Tím se otevře dialogové okno Přidat zobrazení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-349">This will bring up the Add View dialog.</span></span>
...
2. <span data-ttu-id="8b89e-359">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-359">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-360">Změňte adresu URL na **/StoreManager/vytvoření**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-360">Change the URL to **/StoreManager/Create**.</span></span> <span data-ttu-id="8b89e-361">Zkontrolujte, jestli prázdný formulář zobrazuje pro vyplnění nové vlastnosti.</span><span class="sxs-lookup"><span data-stu-id="8b89e-361">Verify that an empty form is displayed for filling the new Album properties.</span></span>
    ...
    <span data-ttu-id="8b89e-363">*Vytvořit zobrazení s prázdný formulář*</span><span class="sxs-lookup"><span data-stu-id="8b89e-363">*Create View with an empty form*</span></span>
...
1. <span data-ttu-id="8b89e-375">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-375">Press **F5** to run the Application.</span></span>
...
1. <span data-ttu-id="8b89e-392">Otevřít **začít** řešení nachází v **zdroj/Ex5-HandlingDeletion/počáteční/** složky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-392">Open the **Begin** solution located at **Source/Ex5-HandlingDeletion/Begin/** folder.</span></span> <span data-ttu-id="8b89e-393">V opačném případě může nadále používat **End** řešení získat provedením předchozím cvičení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-393">Otherwise, you might continue using the **End** solution obtained by completing the previous exercise.</span></span>
   ...
   3. <span data-ttu-id="8b89e-397">Nakonec sestavte řešení kliknutím **sestavení** | **sestavit řešení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-397">Finally, build the solution by clicking **Build** | **Build Solution**.</span></span>
      ...
      > [!NOTE]
      > <span data-ttu-id="8b89e-398">Jednou z výhod pomocí nástroje NuGet je, že není nutné dodávat všechny knihovny ve vašem projektu, zmenšení velikosti projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-398">One of the advantages of using NuGet is that you don't have to ship all the libraries in your project, reducing the project size.</span></span> <span data-ttu-id="8b89e-399">Pomocí nástroje NuGet zadáním verze balíčku v souboru Packages.config, budete moct stáhnout požadované knihovny při prvním spuštění projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-399">With NuGet Power Tools, by specifying the package versions in the Packages.config file, you will be able to download all the required libraries the first time you run the project.</span></span> <span data-ttu-id="8b89e-400">To je důvod, proč budete muset projít tyto kroky po otevření existujícího řešení z tohoto testovacího prostředí.</span><span class="sxs-lookup"><span data-stu-id="8b89e-400">This is why you will have to run these steps after you open an existing solution from this lab.</span></span>
2. <span data-ttu-id="8b89e-401">Otevřít **StoreManagerController** třídy.</span><span class="sxs-lookup"><span data-stu-id="8b89e-401">Open **StoreManagerController** class.</span></span> <span data-ttu-id="8b89e-402">Chcete-li to provést, rozbalte **řadiče** složky a dvojím kliknutím **StoreManagerController.cs**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-402">To do this, expand the **Controllers** folder and double-click **StoreManagerController.cs**.</span></span>
3. <span data-ttu-id="8b89e-403">Odstranění akce kontroleru je přesně stejný jako předchozí akce kontroleru Store podrobnosti: se dotázal **alba** objekt z databáze pomocí **id** součástí adresy URL a vrátí odpovídající **zobrazení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-403">The Delete controller action is exactly the same as the previous Store Details controller action: it queries the **album** object from the database using the **id** provided in the URL and returns the appropriate **View**.</span></span> <span data-ttu-id="8b89e-404">Chcete-li to provést, nahraďte HTTP GET **odstranit** akce metoda kód následujícím kódem:</span><span class="sxs-lookup"><span data-stu-id="8b89e-404">To do this, replace the HTTP-GET **Delete** action method code with the following:</span></span>

    <span data-ttu-id="8b89e-405">(Fragment - kódu *pomocné rutiny ASP.NET MVC 4 a formulářů a ověřování - Ex5 zpracování odstranění – odstranit HTTP GET – akce*)</span><span class="sxs-lookup"><span data-stu-id="8b89e-405">(Code Snippet - *ASP.NET MVC 4 Helpers and Forms and Validation - Ex5 Handling Deletion HTTP-GET Delete action*)</span></span>

    [!code-csharp[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample15.cs)]
4. <span data-ttu-id="8b89e-406">Klepněte pravým tlačítkem myši **odstranit** metody akce a vyberte **přidat zobrazení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-406">Right-click inside the **Delete** action method and select **Add View**.</span></span> <span data-ttu-id="8b89e-407">Tím se otevře dialogové okno Přidat zobrazení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-407">This will bring up the Add View dialog.</span></span>
5. <span data-ttu-id="8b89e-408">V dialogovém okně Přidat zobrazení ověřte, zda je název zobrazení **odstranit**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-408">In the Add View dialog, verify that the View name is **Delete**.</span></span> <span data-ttu-id="8b89e-409">Vyberte **vytvoření zobrazení se silnými typy** možnost a vyberte **alba (MvcMusicStore.Models)** z **třída modelu** rozevíracího seznamu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-409">Select the **Create a strongly-typed view** option and select **Album (MvcMusicStore.Models)** from the **Model class** drop-down.</span></span> <span data-ttu-id="8b89e-410">Vyberte **odstranit** z **šablona Scaffold** rozevíracího seznamu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-410">Select **Delete** from the **Scaffold template** drop-down.</span></span> <span data-ttu-id="8b89e-411">Ostatní pole nechte výchozí hodnoty a pak klikněte na tlačítko **přidat**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-411">Leave the other fields with their default value and then click **Add**.</span></span>

    <span data-ttu-id="8b89e-412">![Přidání zobrazení odstranit](aspnet-mvc-4-helpers-forms-and-validation/_static/image15.png "přidání odstranit zobrazení")</span><span class="sxs-lookup"><span data-stu-id="8b89e-412">![Adding a Delete View](aspnet-mvc-4-helpers-forms-and-validation/_static/image15.png "Adding a Delete View")</span></span>

    <span data-ttu-id="8b89e-413">*Přidání zobrazení Delete*</span><span class="sxs-lookup"><span data-stu-id="8b89e-413">*Adding a Delete View*</span></span>
6. <span data-ttu-id="8b89e-414">Odstranit šablonu zobrazí všechna pole z modelu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-414">The Delete template shows all the fields from the model.</span></span> <span data-ttu-id="8b89e-415">Se zobrazí pouze alba název.</span><span class="sxs-lookup"><span data-stu-id="8b89e-415">You will show only the album's title.</span></span> <span data-ttu-id="8b89e-416">Provedete to tak, nahradí obsah zobrazení s následujícím kódem:</span><span class="sxs-lookup"><span data-stu-id="8b89e-416">To do this, replace the content of the view with the following code:</span></span>
...
#### <a name="task-2---running-the-application"></a><span data-ttu-id="8b89e-417">Úloha 2 – spuštění aplikace</span><span class="sxs-lookup"><span data-stu-id="8b89e-417">Task 2 - Running the Application</span></span>

<span data-ttu-id="8b89e-418">V této úloze budete testovat, který **StoreManager** **odstranit** zobrazení stránky zobrazí formulář potvrzení odstranění.</span><span class="sxs-lookup"><span data-stu-id="8b89e-418">In this task, you will test that the **StoreManager** **Delete** View page displays a confirmation deletion form.</span></span>

1. <span data-ttu-id="8b89e-419">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-419">Press **F5** to run the Application.</span></span>
2. <span data-ttu-id="8b89e-420">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-420">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-421">Změňte adresu URL na **/StoreManager**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-421">Change the URL to **/StoreManager**.</span></span> <span data-ttu-id="8b89e-422">Vyberte jeden alba odstranit tak, že kliknete na **odstranit** a ověřte, že se nahraje nové zobrazení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-422">Select one album to delete by clicking **Delete** and verify that the new view is uploaded.</span></span>

    <span data-ttu-id="8b89e-423">![Odstraňuje se alba](aspnet-mvc-4-helpers-forms-and-validation/_static/image16.png "odstranění alba")</span><span class="sxs-lookup"><span data-stu-id="8b89e-423">![Deleting an Album](aspnet-mvc-4-helpers-forms-and-validation/_static/image16.png "Deleting an Album")</span></span>

    <span data-ttu-id="8b89e-424">*Odstraňuje se alba*</span><span class="sxs-lookup"><span data-stu-id="8b89e-424">*Deleting an Album*</span></span>

<a id="Ex05Task3"></a>

<a id="Task_3-_Implementing_the_HTTP-POST_Delete_Action_Method"></a>
#### <a name="task-3--implementing-the-http-post-delete-action-method"></a><span data-ttu-id="8b89e-425">Úloha 3 – implementace metody akce Delete HTTP POST</span><span class="sxs-lookup"><span data-stu-id="8b89e-425">Task 3- Implementing the HTTP-POST Delete Action Method</span></span>

<span data-ttu-id="8b89e-426">V této úloze budete implementovat verze HTTP POST metody akce odstranění, který bude vyvolán při kliknutí **odstranit** tlačítko.</span><span class="sxs-lookup"><span data-stu-id="8b89e-426">In this task, you will implement the HTTP-POST version of the Delete action method that will be invoked when a user clicks the **Delete** button.</span></span> <span data-ttu-id="8b89e-427">Metoda by měla odstranit album v databázi.</span><span class="sxs-lookup"><span data-stu-id="8b89e-427">The method should delete the album in the database.</span></span>

1. <span data-ttu-id="8b89e-428">Zavřete prohlížeč v případě potřeby se vraťte do okna nástroje Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="8b89e-428">Close the browser if needed, to return to the Visual Studio window.</span></span> <span data-ttu-id="8b89e-429">Otevřít **StoreManagerController** třídy.</span><span class="sxs-lookup"><span data-stu-id="8b89e-429">Open **StoreManagerController** class.</span></span> <span data-ttu-id="8b89e-430">Chcete-li to provést, rozbalte **řadiče** složky a dvojím kliknutím **StoreManagerController.cs**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-430">To do this, expand the **Controllers** folder and double-click **StoreManagerController.cs**.</span></span>
2. <span data-ttu-id="8b89e-431">Nahraďte **HTTP POST odstranit** akce metoda kód následujícím kódem:</span><span class="sxs-lookup"><span data-stu-id="8b89e-431">Replace **HTTP-POST Delete** action method code with the following:</span></span>

    <span data-ttu-id="8b89e-432">(Fragment - kódu *pomocné rutiny ASP.NET MVC 4 a formulářů a ověřování - Ex5 zpracování odstranění – odstranit HTTP POST – akce*)</span><span class="sxs-lookup"><span data-stu-id="8b89e-432">(Code Snippet - *ASP.NET MVC 4 Helpers and Forms and Validation - Ex5 Handling Deletion HTTP-POST Delete action*)</span></span>
...
#### <a name="task-4---running-the-application"></a><span data-ttu-id="8b89e-433">Úloha 4 – spouštění aplikace</span><span class="sxs-lookup"><span data-stu-id="8b89e-433">Task 4 - Running the Application</span></span>

<span data-ttu-id="8b89e-434">V této úloze budete testovat, který **StoreManager odstranit** stránka zobrazení můžete odstranit alba a pak přesměruje na zobrazení indexu StoreManager.</span><span class="sxs-lookup"><span data-stu-id="8b89e-434">In this task, you will test that the **StoreManager Delete** View page lets you delete an Album and then redirects to the StoreManager Index View.</span></span>

1. <span data-ttu-id="8b89e-435">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-435">Press **F5** to run the Application.</span></span>
2. <span data-ttu-id="8b89e-436">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-436">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-437">Změňte adresu URL na **/StoreManager**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-437">Change the URL to **/StoreManager**.</span></span> <span data-ttu-id="8b89e-438">Vyberte jeden alba odstranit tak, že kliknete na **odstranit.**</span><span class="sxs-lookup"><span data-stu-id="8b89e-438">Select one album to delete by clicking **Delete.**</span></span> <span data-ttu-id="8b89e-439">Potvrďte odstranění kliknutím **odstranit** tlačítka:</span><span class="sxs-lookup"><span data-stu-id="8b89e-439">Confirm the deletion by clicking **Delete** button:</span></span>

    <span data-ttu-id="8b89e-440">![Odstraňuje se alba](aspnet-mvc-4-helpers-forms-and-validation/_static/image17.png "odstranění alba")</span><span class="sxs-lookup"><span data-stu-id="8b89e-440">![Deleting an Album](aspnet-mvc-4-helpers-forms-and-validation/_static/image17.png "Deleting an Album")</span></span>

    <span data-ttu-id="8b89e-441">*Odstraňuje se alba*</span><span class="sxs-lookup"><span data-stu-id="8b89e-441">*Deleting an Album*</span></span>
3. <span data-ttu-id="8b89e-442">Ověřte, že byl alba odstranit, protože se nezobrazují v **Index** stránky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-442">Verify that the album was deleted since it does not appear in the **Index** page.</span></span>

<a id="Exercise6"></a>

<a id="Exercise_6_Adding_Validation"></a>
### <a name="exercise-6-adding-validation"></a><span data-ttu-id="8b89e-443">Cvičení 6: Přidání ověření</span><span class="sxs-lookup"><span data-stu-id="8b89e-443">Exercise 6: Adding Validation</span></span>

<span data-ttu-id="8b89e-444">Vytvoření a úprava formuláře, které máte v současné době neprovádějte jakéhokoli druhu ověřování.</span><span class="sxs-lookup"><span data-stu-id="8b89e-444">Currently, the Create and Edit forms you have in place do not perform any kind of validation.</span></span> <span data-ttu-id="8b89e-445">Pokud uživatel ponechá povinné pole je prázdné nebo písmena typ v poli pro cenu, první chyby, které dostanete se z databáze.</span><span class="sxs-lookup"><span data-stu-id="8b89e-445">If the user leaves a required field blank or type letters in the price field, the first error you will get will be from the database.</span></span>

<span data-ttu-id="8b89e-446">Můžete přidat ověřování do aplikace tak, že přidáte anotacemi dat do vaší třídy modelu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-446">You can add validation to the application by adding Data Annotations to your model class.</span></span> <span data-ttu-id="8b89e-447">Anotací dat při povolení popisující pravidla, která chcete použít pro vlastnosti modelu a ASP.NET MVC se postará o vynucení a zobrazení odpovídající zprávu pro uživatele.</span><span class="sxs-lookup"><span data-stu-id="8b89e-447">Data Annotations allow describing the rules you want applied to your model properties, and ASP.NET MVC will take care of enforcing and displaying appropriate message to users.</span></span>

<a id="Ex06Task1"></a>

<a id="Task_1_-_Adding_Data_Annotations"></a>
#### <a name="task-1---adding-data-annotations"></a><span data-ttu-id="8b89e-448">Úloha 1 – přidání anotací dat</span><span class="sxs-lookup"><span data-stu-id="8b89e-448">Task 1 - Adding Data Annotations</span></span>

<span data-ttu-id="8b89e-449">V této úloze přidáte anotacemi dat alba model, který bude na stránce vytvořit a upravit zobrazení ověřovacích zpráv v případě potřeby.</span><span class="sxs-lookup"><span data-stu-id="8b89e-449">In this task, you will add Data Annotations to the Album Model that will make the Create and Edit page display validation messages when appropriate.</span></span>

<span data-ttu-id="8b89e-450">Pro jednoduchou třídu modelu, přidávání poznámek Data právě probíhá tak, že přidáte **pomocí** příkaz pro **System.ComponentModel.DataAnnotation**, pak umístit **[povinné]** atribut na odpovídající vlastnosti.</span><span class="sxs-lookup"><span data-stu-id="8b89e-450">For a simple Model class, adding a Data Annotation is just handled by adding a **using** statement for **System.ComponentModel.DataAnnotation**, then placing a **[Required]** attribute on the appropriate properties.</span></span> <span data-ttu-id="8b89e-451">V následujícím příkladu s žádným **název** vlastnost povinné pole. v zobrazení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-451">The following example would make the **Name** property a required field in the View.</span></span>

[!code-csharp[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample18.cs)]

<span data-ttu-id="8b89e-452">Toto je o něco složitější v případech, jako je tato aplikace, ve kterém se vygeneruje modelu Entity Data Model.</span><span class="sxs-lookup"><span data-stu-id="8b89e-452">This is a little more complex in cases like this application where the Entity Data Model is generated.</span></span> <span data-ttu-id="8b89e-453">Pokud jste přidali anotacemi dat přímo do třídy modelu, byste při aktualizaci modelu z databáze nástroje přepsány.</span><span class="sxs-lookup"><span data-stu-id="8b89e-453">If you added Data Annotations directly to the model classes, they would be overwritten if you update the model from the database.</span></span> <span data-ttu-id="8b89e-454">Místo toho můžete provést pomocí metadat částečné třídy, které budou existovat pro uložení poznámky a jsou spojeny s modelem třídy pomocí **[MetadataType]** atribut.</span><span class="sxs-lookup"><span data-stu-id="8b89e-454">Instead, you can make use of metadata partial classes which will exist to hold the annotations and are associated with the model classes using the **[MetadataType]** attribute.</span></span>

1. <span data-ttu-id="8b89e-455">Otevřít **začít** řešení nachází v **zdroj/Ex6-AddingValidation/počáteční/** složky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-455">Open the **Begin** solution located at **Source/Ex6-AddingValidation/Begin/** folder.</span></span> <span data-ttu-id="8b89e-456">V opačném případě může nadále používat **End** řešení získat provedením předchozím cvičení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-456">Otherwise, you might continue using the **End** solution obtained by completing the previous exercise.</span></span>

   1. <span data-ttu-id="8b89e-457">Pokud jste otevřeli zadaných **začít** řešení, budete muset stáhnout některé chybějící balíčky NuGet než budete pokračovat.</span><span class="sxs-lookup"><span data-stu-id="8b89e-457">If you opened the provided **Begin** solution, you will need to download some missing NuGet packages before continue.</span></span> <span data-ttu-id="8b89e-458">Chcete-li to provést, klikněte na tlačítko **projektu** nabídky a vybereme **spravovat balíčky NuGet**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-458">To do this, click the **Project** menu and select **Manage NuGet Packages**.</span></span>
   2. <span data-ttu-id="8b89e-459">V **spravovat balíčky NuGet** dialogového okna, klikněte na tlačítko **obnovení** aby bylo možné stáhnout chybějící balíčky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-459">In the **Manage NuGet Packages** dialog, click **Restore** in order to download missing packages.</span></span>
   3. <span data-ttu-id="8b89e-460">Nakonec sestavte řešení kliknutím **sestavení** | **sestavit řešení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-460">Finally, build the solution by clicking **Build** | **Build Solution**.</span></span>

      > [!NOTE]
      > <span data-ttu-id="8b89e-461">Jednou z výhod pomocí nástroje NuGet je, že není nutné dodávat všechny knihovny ve vašem projektu, zmenšení velikosti projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-461">One of the advantages of using NuGet is that you don't have to ship all the libraries in your project, reducing the project size.</span></span> <span data-ttu-id="8b89e-462">Pomocí nástroje NuGet zadáním verze balíčku v souboru Packages.config, budete moct stáhnout požadované knihovny při prvním spuštění projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-462">With NuGet Power Tools, by specifying the package versions in the Packages.config file, you will be able to download all the required libraries the first time you run the project.</span></span> <span data-ttu-id="8b89e-463">To je důvod, proč budete muset projít tyto kroky po otevření existujícího řešení z tohoto testovacího prostředí.</span><span class="sxs-lookup"><span data-stu-id="8b89e-463">This is why you will have to run these steps after you open an existing solution from this lab.</span></span>
2. <span data-ttu-id="8b89e-464">Otevřít **Album.cs** z **modely** složky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-464">Open the **Album.cs** from the **Models** folder.</span></span>
3. <span data-ttu-id="8b89e-465">Nahraďte **Album.cs** obsahu se zvýrazněný kód tak, aby vypadal jako následující:</span><span class="sxs-lookup"><span data-stu-id="8b89e-465">Replace **Album.cs** content with the highlighted code, so that it looks like the following:</span></span>

    > [!NOTE]
    > <span data-ttu-id="8b89e-466">Na řádku **[DisplayFormat(ConvertEmptyStringToNull=false)]** označuje, že prázdné řetězce z modelu nebudou převedeny na hodnotu null při aktualizaci ve zdroji dat datové pole.</span><span class="sxs-lookup"><span data-stu-id="8b89e-466">The line **[DisplayFormat(ConvertEmptyStringToNull=false)]** indicates that empty strings from the model won't be converted to null when the data field is updated in the data source.</span></span> <span data-ttu-id="8b89e-467">Toto nastavení zabrání výjimku, pokud předtím, než Data anotace ověřuje pole Entity Framework přiřadí hodnoty null do modelu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-467">This setting will avoid an exception when the Entity Framework assigns null values to the model before Data Annotation validates the fields.</span></span>

    <span data-ttu-id="8b89e-468">(Fragment - kódu *pomocné rutiny ASP.NET MVC 4 a formulářů a ověřování - částečné třídy metadat Ex6 alba*)</span><span class="sxs-lookup"><span data-stu-id="8b89e-468">(Code Snippet - *ASP.NET MVC 4 Helpers and Forms and Validation - Ex6 Album metadata partial class*)</span></span>

    [!code-csharp[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample19.cs)]

    > [!NOTE]
    > <span data-ttu-id="8b89e-469">To **alba** má částečné třídy **MetadataType** atribut, který odkazuje na **AlbumMetaData** třídy pro datové poznámky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-469">This **Album** partial class has a **MetadataType** attribute which points to the **AlbumMetaData** class for the Data Annotations.</span></span> <span data-ttu-id="8b89e-470">Zde je několik příkladů dat anotace atributy, které používáte opatřit poznámkami alba modelu:</span><span class="sxs-lookup"><span data-stu-id="8b89e-470">These are some of the Data Annotation attributes you are using to annotate the Album model:</span></span>
    > 
    > - <span data-ttu-id="8b89e-471">Nepožadováno – označuje, že vlastnost je povinné pole.</span><span class="sxs-lookup"><span data-stu-id="8b89e-471">Required - Indicates that the property is a required field</span></span>
    > - <span data-ttu-id="8b89e-472">DisplayName – definuje text bude použit na pole formuláře a ověřovacích zpráv</span><span class="sxs-lookup"><span data-stu-id="8b89e-472">DisplayName - Defines the text to be used on form fields and validation messages</span></span>
    > - <span data-ttu-id="8b89e-473">DisplayFormat – určuje způsob zobrazení a ve formátu datová pole.</span><span class="sxs-lookup"><span data-stu-id="8b89e-473">DisplayFormat - Specifies how data fields are displayed and formatted.</span></span>
    > - <span data-ttu-id="8b89e-474">StringLength – definuje maximální délku pole řetězce</span><span class="sxs-lookup"><span data-stu-id="8b89e-474">StringLength - Defines a maximum length for a string field</span></span>
    > - <span data-ttu-id="8b89e-475">V rozsahu - poskytuje maximální a minimální hodnoty pro číselné pole</span><span class="sxs-lookup"><span data-stu-id="8b89e-475">Range - Gives a maximum and minimum value for a numeric field</span></span>
    > - <span data-ttu-id="8b89e-476">ScaffoldColumn – umožňuje skrýt pole z editoru formuláře</span><span class="sxs-lookup"><span data-stu-id="8b89e-476">ScaffoldColumn - Allows hiding fields from editor forms</span></span>

<a id="Ex06Task2"></a>

<a id="Task_2_-_Running_the_Application"></a>
#### <a name="task-2---running-the-application"></a><span data-ttu-id="8b89e-477">Úloha 2 – spuštění aplikace</span><span class="sxs-lookup"><span data-stu-id="8b89e-477">Task 2 - Running the Application</span></span>

<span data-ttu-id="8b89e-478">V této úloze budete testovat, vytvoření a úprava stránky ověření pole, pomocí zobrazované názvy zvolili v minulé úloze.</span><span class="sxs-lookup"><span data-stu-id="8b89e-478">In this task, you will test that the Create and Edit pages validate fields, using the display names chosen in the last task.</span></span>

1. <span data-ttu-id="8b89e-479">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-479">Press **F5** to run the Application.</span></span>
2. <span data-ttu-id="8b89e-480">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-480">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-481">Změňte adresu URL na **/StoreManager/vytvoření**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-481">Change the URL to **/StoreManager/Create**.</span></span> <span data-ttu-id="8b89e-482">Ověřte, že zobrazované názvy odpovídají v částečné třídy (jako je **alba obrázky URL** místo **AlbumArtUrl**)</span><span class="sxs-lookup"><span data-stu-id="8b89e-482">Verify that the display names match the ones in the partial class (like **Album Art URL** instead of **AlbumArtUrl**)</span></span>
3. <span data-ttu-id="8b89e-483">Klikněte na tlačítko **vytvořit**, bez vyplňování formuláře.</span><span class="sxs-lookup"><span data-stu-id="8b89e-483">Click **Create**, without filling the form.</span></span> <span data-ttu-id="8b89e-484">Ověřte, že dostanete odpovídajících ověřovacích zpráv.</span><span class="sxs-lookup"><span data-stu-id="8b89e-484">Verify that you get the corresponding validation messages.</span></span>

    <span data-ttu-id="8b89e-485">![Ověření pole na stránce vytvořit](aspnet-mvc-4-helpers-forms-and-validation/_static/image18.png "ověření pole na stránce vytvořit")</span><span class="sxs-lookup"><span data-stu-id="8b89e-485">![Validated fields in the Create page](aspnet-mvc-4-helpers-forms-and-validation/_static/image18.png "Validated fields in the Create page")</span></span>

    <span data-ttu-id="8b89e-486">*Ověřená polí na stránce vytvořit*</span><span class="sxs-lookup"><span data-stu-id="8b89e-486">*Validated fields in the Create page*</span></span>
4. <span data-ttu-id="8b89e-487">Můžete ověřit, že stejné dochází u **upravit** stránky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-487">You can verify that the same occurs with the **Edit** page.</span></span> <span data-ttu-id="8b89e-488">Změňte adresu URL na **/StoreManager/Edit/1** a ověřte, že zobrazované názvy odpovídají v částečné třídy (jako je **alba obrázky URL** místo **AlbumArtUrl**).</span><span class="sxs-lookup"><span data-stu-id="8b89e-488">Change the URL to **/StoreManager/Edit/1** and verify that the display names match the ones in the partial class (like **Album Art URL** instead of **AlbumArtUrl**).</span></span> <span data-ttu-id="8b89e-489">Prázdný **Title** a **cena** pole a klikněte na tlačítko **Uložit**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-489">Empty the **Title** and **Price** fields and click **Save**.</span></span> <span data-ttu-id="8b89e-490">Ověřte, že dostanete odpovídajících ověřovacích zpráv.</span><span class="sxs-lookup"><span data-stu-id="8b89e-490">Verify that you get the corresponding validation messages.</span></span>

    ![Ověřená polí na stránce Upravit](aspnet-mvc-4-helpers-forms-and-validation/_static/image19.png)

    <span data-ttu-id="8b89e-492">*Ověřená polí na stránce Upravit*</span><span class="sxs-lookup"><span data-stu-id="8b89e-492">*Validated fields in the Edit page*</span></span>

<a id="Exercise7"></a>

<a id="Exercise_7_Using_Unobtrusive_jQuery_at_Client_Side"></a>
### <a name="exercise-7-using-unobtrusive-jquery-at-client-side"></a><span data-ttu-id="8b89e-493">Cvičení 7: Pomocí Nerušivého jQuery na straně klienta</span><span class="sxs-lookup"><span data-stu-id="8b89e-493">Exercise 7: Using Unobtrusive jQuery at Client Side</span></span>

<span data-ttu-id="8b89e-494">V tomto cvičení se dozvíte, jak povolit MVC 4 Nerušivý jQuery ověření na straně klienta.</span><span class="sxs-lookup"><span data-stu-id="8b89e-494">In this exercise, you will learn how to enable MVC 4 Unobtrusive jQuery validation at client side.</span></span>

> [!NOTE]
> <span data-ttu-id="8b89e-495">Nerušivý jQuery používá data-ajax předponu JavaScript volat v serveru místo intrusively generování skriptů klienta vložené metody akce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-495">The Unobtrusive jQuery uses data-ajax prefix JavaScript to invoke action methods on the server rather than intrusively emitting inline client scripts.</span></span>

<a id="Ex7Task1"></a>

<a id="Task_1_-_Running_the_Application_before_Enabling_Unobtrusive_jQuery"></a>
#### <a name="task-1---running-the-application-before-enabling-unobtrusive-jquery"></a><span data-ttu-id="8b89e-496">Úloha 1 – spuštění aplikace před povolením Nerušivý jQuery</span><span class="sxs-lookup"><span data-stu-id="8b89e-496">Task 1 - Running the Application before Enabling Unobtrusive jQuery</span></span>

<span data-ttu-id="8b89e-497">V této úloze budete spouštět aplikace před zahrnutím jQuery k porovnání obou modelů ověřování.</span><span class="sxs-lookup"><span data-stu-id="8b89e-497">In this task, you will run the application before including jQuery in order to compare both validation models.</span></span>

1. <span data-ttu-id="8b89e-498">Otevřít **začít** řešení nachází v **zdroj/Ex7-UnobtrusivejQueryValidation/počáteční/** složky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-498">Open the **Begin** solution located at **Source/Ex7-UnobtrusivejQueryValidation/Begin/** folder.</span></span> <span data-ttu-id="8b89e-499">V opačném případě může nadále používat **End** řešení získat provedením předchozím cvičení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-499">Otherwise, you might continue using the **End** solution obtained by completing the previous exercise.</span></span>

   1. <span data-ttu-id="8b89e-500">Pokud jste otevřeli zadaných **začít** řešení, budete muset stáhnout některé chybějící balíčky NuGet než budete pokračovat.</span><span class="sxs-lookup"><span data-stu-id="8b89e-500">If you opened the provided **Begin** solution, you will need to download some missing NuGet packages before continue.</span></span> <span data-ttu-id="8b89e-501">Chcete-li to provést, klikněte na tlačítko **projektu** nabídky a vybereme **spravovat balíčky NuGet**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-501">To do this, click the **Project** menu and select **Manage NuGet Packages**.</span></span>
   2. <span data-ttu-id="8b89e-502">V **spravovat balíčky NuGet** dialogového okna, klikněte na tlačítko **obnovení** aby bylo možné stáhnout chybějící balíčky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-502">In the **Manage NuGet Packages** dialog, click **Restore** in order to download missing packages.</span></span>
   3. <span data-ttu-id="8b89e-503">Nakonec sestavte řešení kliknutím **sestavení** | **sestavit řešení**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-503">Finally, build the solution by clicking **Build** | **Build Solution**.</span></span>

      > [!NOTE]
      > <span data-ttu-id="8b89e-504">Jednou z výhod pomocí nástroje NuGet je, že není nutné dodávat všechny knihovny ve vašem projektu, zmenšení velikosti projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-504">One of the advantages of using NuGet is that you don't have to ship all the libraries in your project, reducing the project size.</span></span> <span data-ttu-id="8b89e-505">Pomocí nástroje NuGet zadáním verze balíčku v souboru Packages.config, budete moct stáhnout požadované knihovny při prvním spuštění projektu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-505">With NuGet Power Tools, by specifying the package versions in the Packages.config file, you will be able to download all the required libraries the first time you run the project.</span></span> <span data-ttu-id="8b89e-506">To je důvod, proč budete muset projít tyto kroky po otevření existujícího řešení z tohoto testovacího prostředí.</span><span class="sxs-lookup"><span data-stu-id="8b89e-506">This is why you will have to run these steps after you open an existing solution from this lab.</span></span>
2. <span data-ttu-id="8b89e-507">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-507">Press **F5** to run the application.</span></span>
3. <span data-ttu-id="8b89e-508">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-508">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-509">Procházet **/StoreManager/vytvořit** a klikněte na tlačítko **vytvořit** bez vyplňování formuláře a přesvědčte se, že dostanete zprávy ověření na:</span><span class="sxs-lookup"><span data-stu-id="8b89e-509">Browse **/StoreManager/Create** and click **Create** without filling the form to verify that you get validation messages:</span></span>

    <span data-ttu-id="8b89e-510">![Zakázáno ověřování na straně klienta](aspnet-mvc-4-helpers-forms-and-validation/_static/image20.png "zakázáno ověřování na straně klienta")</span><span class="sxs-lookup"><span data-stu-id="8b89e-510">![Client validation disabled](aspnet-mvc-4-helpers-forms-and-validation/_static/image20.png "Client validation disabled")</span></span>

    <span data-ttu-id="8b89e-511">*Zakázáno ověřování na straně klienta*</span><span class="sxs-lookup"><span data-stu-id="8b89e-511">*Client validation disabled*</span></span>
4. <span data-ttu-id="8b89e-512">V prohlížeči otevřete zdrojový kód HTML:</span><span class="sxs-lookup"><span data-stu-id="8b89e-512">In the browser, open the HTML source code:</span></span>
...
#### <a name="task-2---enabling-unobtrusive-client-validation"></a><span data-ttu-id="8b89e-513">Úloha 2 – povolení ověřování Nerušivý klienta</span><span class="sxs-lookup"><span data-stu-id="8b89e-513">Task 2 - Enabling Unobtrusive Client Validation</span></span>

<span data-ttu-id="8b89e-514">V této úloze povolíte jQuery **ověření nerušivého klienta** z **Web.config** soubor, který je ve výchozím nastavení nastavena na hodnotu false ve všech nových projektech ASP.NET MVC 4.</span><span class="sxs-lookup"><span data-stu-id="8b89e-514">In this task, you will enable jQuery **unobtrusive client validation** from **Web.config** file, which is by default set to false in all new ASP.NET MVC 4 projects.</span></span> <span data-ttu-id="8b89e-515">Kromě toho můžete přidá že nezbytné skripty odkazy, aby jQuery pracovní Nerušivý ověřování na straně klienta.</span><span class="sxs-lookup"><span data-stu-id="8b89e-515">Additionally, you will add the necessary scripts references to make jQuery Unobtrusive Client Validation work.</span></span>

1. <span data-ttu-id="8b89e-516">Otevřít **Web.Config** soubor v kořenu projektu a ujistěte se, že **ClientValidationEnabled** a **UnobtrusiveJavaScriptEnabled** hodnoty klíče jsou nastaveny na **true**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-516">Open **Web.Config** file at project root, and make sure that the **ClientValidationEnabled** and **UnobtrusiveJavaScriptEnabled** keys values are set to **true**.</span></span>

    [!code-xml[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample21.xml)]

    > [!NOTE]
    > <span data-ttu-id="8b89e-517">Můžete také povolit ověřování na straně klienta pomocí kódu v Global.asax.cs stejné výsledky:</span><span class="sxs-lookup"><span data-stu-id="8b89e-517">You can also enable client validation by code at Global.asax.cs to get the same results:</span></span>
    > 
    > <span data-ttu-id="8b89e-518">**HtmlHelper.ClientValidationEnabled = true;**</span><span class="sxs-lookup"><span data-stu-id="8b89e-518">**HtmlHelper.ClientValidationEnabled = true;**</span></span>
    > 
    > <span data-ttu-id="8b89e-519">Kromě toho můžete přiřadit ClientValidationEnabled atribut do libovolného řadiče mít vlastní chování.</span><span class="sxs-lookup"><span data-stu-id="8b89e-519">Additionally, you can assign ClientValidationEnabled attribute into any controller to have a custom behavior.</span></span>
2. <span data-ttu-id="8b89e-520">Otevřít **Create.cshtml** na **Views\StoreManager**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-520">Open **Create.cshtml** at **Views\StoreManager**.</span></span>
3. <span data-ttu-id="8b89e-521">Ujistěte se, že následující soubory skriptu, **jquery.validate** a **jquery.validate.unobtrusive**, jsou odkazovány v zobrazení prostřednictvím &quot; **~/bundles/jqueryval** &quot; sady.</span><span class="sxs-lookup"><span data-stu-id="8b89e-521">Make sure the following script files, **jquery.validate** and **jquery.validate.unobtrusive**, are referenced in the view through the &quot;**~/bundles/jqueryval**&quot; bundle.</span></span>

    [!code-cshtml[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample22.cshtml)]

    > [!NOTE]
    > <span data-ttu-id="8b89e-522">Tyto knihovny jQuery jsou součástí nové projekty MVC 4.</span><span class="sxs-lookup"><span data-stu-id="8b89e-522">All these jQuery libraries are included in MVC 4 new projects.</span></span> <span data-ttu-id="8b89e-523">Můžete najít další knihovny v **/skripty** složce projektu je.</span><span class="sxs-lookup"><span data-stu-id="8b89e-523">You can find more libraries in the **/Scripts** folder of you project.</span></span>
    > 
    > <span data-ttu-id="8b89e-524">Aby bylo toto ověření pracovní knihovny, budete muset přidat odkaz na framework knihovnu jQuery.</span><span class="sxs-lookup"><span data-stu-id="8b89e-524">In order to make this validation libraries work, you need to add a reference to the jQuery framework library.</span></span> <span data-ttu-id="8b89e-525">Vzhledem k tomu, že tento odkaz je již přidána do  **\_Layout.cshtml** souboru, není potřeba ji přidat do tohoto zobrazení.</span><span class="sxs-lookup"><span data-stu-id="8b89e-525">Since this reference is already added in the **\_Layout.cshtml** file, you do not need to add it in this particular view.</span></span>

<a id="Ex7Task3"></a>

<a id="Task_3_-_Running_the_Application_Using_Unobtrusive_jQuery_Validation"></a>
#### <a name="task-3---running-the-application-using-unobtrusive-jquery-validation"></a><span data-ttu-id="8b89e-526">Úloha 3 – spuštění aplikace pomocí Nerušivého jQuery ověření</span><span class="sxs-lookup"><span data-stu-id="8b89e-526">Task 3 - Running the Application Using Unobtrusive jQuery Validation</span></span>

<span data-ttu-id="8b89e-527">V této úloze budete testovat, který **StoreManager** vytvořte zobrazení šablony provádí ověřování na straně klienta pomocí knihovny jQuery, když uživatel vytvoří nové album.</span><span class="sxs-lookup"><span data-stu-id="8b89e-527">In this task, you will test that the **StoreManager** create view template performs client side validation using jQuery libraries when the user creates a new album.</span></span>

1. <span data-ttu-id="8b89e-528">Stisknutím klávesy **F5** ke spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="8b89e-528">Press **F5** to run the application.</span></span>
2. <span data-ttu-id="8b89e-529">Projekt se spustí na domovské stránce.</span><span class="sxs-lookup"><span data-stu-id="8b89e-529">The project starts in the Home page.</span></span> <span data-ttu-id="8b89e-530">Procházet **/StoreManager/vytvořit** a klikněte na tlačítko **vytvořit** bez vyplňování formuláře a přesvědčte se, že dostanete zprávy ověření na:</span><span class="sxs-lookup"><span data-stu-id="8b89e-530">Browse **/StoreManager/Create** and click **Create** without filling the form to verify that you get validation messages:</span></span>

    <span data-ttu-id="8b89e-531">![Ověřování na straně klienta s jQuery povolené](aspnet-mvc-4-helpers-forms-and-validation/_static/image21.png "ověřování na straně klienta s jQuery povoleno")</span><span class="sxs-lookup"><span data-stu-id="8b89e-531">![Client validation with jQuery enabled](aspnet-mvc-4-helpers-forms-and-validation/_static/image21.png "Client validation with jQuery enabled")</span></span>

    <span data-ttu-id="8b89e-532">*Ověřování na straně klienta s jQuery povoleno*</span><span class="sxs-lookup"><span data-stu-id="8b89e-532">*Client validation with jQuery enabled*</span></span>
3. <span data-ttu-id="8b89e-533">V prohlížeči otevřete zdrojový kód pro zobrazení pro vytváření:</span><span class="sxs-lookup"><span data-stu-id="8b89e-533">In the browser, open the source code for Create view:</span></span>

    [!code-html[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample23.html)]

   > [!NOTE]
   > <span data-ttu-id="8b89e-534">Pro každý ověřovací pravidlo klienta Nerušivý jQuery přidá atribut s daty – val –*rulename*=&quot;*zpráva*&quot;.</span><span class="sxs-lookup"><span data-stu-id="8b89e-534">For each client validation rule, Unobtrusive jQuery adds an attribute with data-val-*rulename*=&quot;*message*&quot;.</span></span> <span data-ttu-id="8b89e-535">Níže je seznam značek tohoto Unobtrusive jQuery vloží do vstupního pole html k provedení ověřování na straně klienta:</span><span class="sxs-lookup"><span data-stu-id="8b89e-535">Below is a list of tags that Unobtrusive jQuery inserts into the html input field to perform client validation:</span></span>
   > 
   > - <span data-ttu-id="8b89e-536">Data val</span><span class="sxs-lookup"><span data-stu-id="8b89e-536">Data-val</span></span>
   > - <span data-ttu-id="8b89e-537">Číslo datového val</span><span class="sxs-lookup"><span data-stu-id="8b89e-537">Data-val-number</span></span>
   > - <span data-ttu-id="8b89e-538">Oblast dat val</span><span class="sxs-lookup"><span data-stu-id="8b89e-538">Data-val-range</span></span>
   > - <span data-ttu-id="8b89e-539">Data val rozsah min / Data val range max</span><span class="sxs-lookup"><span data-stu-id="8b89e-539">Data-val-range-min / Data-val-range-max</span></span>
   > - <span data-ttu-id="8b89e-540">Vyžadována val data</span><span class="sxs-lookup"><span data-stu-id="8b89e-540">Data-val-required</span></span>
   > - <span data-ttu-id="8b89e-541">Délka dat val</span><span class="sxs-lookup"><span data-stu-id="8b89e-541">Data-val-length</span></span>
   > - <span data-ttu-id="8b89e-542">Data-val délka max / dat val délka min</span><span class="sxs-lookup"><span data-stu-id="8b89e-542">Data-val-length-max / Data-val-length-min</span></span>
   > 
   > <span data-ttu-id="8b89e-543">Všechny datové hodnoty jsou vyplněny hodnotou modelu **dat. Poznámka**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-543">All the data values are filled with model **Data Annotation**.</span></span> <span data-ttu-id="8b89e-544">Potom veškerou logiku, která funguje na straně serveru může být spuštěna na straně klienta.</span><span class="sxs-lookup"><span data-stu-id="8b89e-544">Then, all the logic that works at server side can be run at client side.</span></span> <span data-ttu-id="8b89e-545">Například cena atribut má následující poznámka data v modelu:</span><span class="sxs-lookup"><span data-stu-id="8b89e-545">For example, Price attribute has the following data annotation in the model:</span></span>
   > 
   > [!code-csharp[Main](aspnet-mvc-4-helpers-forms-and-validation/samples/sample24.cs)]
   > 
   > <span data-ttu-id="8b89e-546">Po použití Nerušivý jQuery, je generovaný kód:</span><span class="sxs-lookup"><span data-stu-id="8b89e-546">After using Unobtrusive jQuery, the generated code is:</span></span>
   > 
...
<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="8b89e-547">Souhrn</span><span class="sxs-lookup"><span data-stu-id="8b89e-547">Summary</span></span>

<span data-ttu-id="8b89e-548">Vyplněním tohoto praktického testovacího prostředí jste se naučili, jak povolit uživatelům změnit data uložená v databázi s použitím následující:</span><span class="sxs-lookup"><span data-stu-id="8b89e-548">By completing this Hands-On Lab you have learned how to enable users to change the data stored in the database with the use of the following:</span></span>

- <span data-ttu-id="8b89e-549">Akce kontroleru, jako je Index, vytvořit, upravit, odstranit</span><span class="sxs-lookup"><span data-stu-id="8b89e-549">Controller actions like Index, Create, Edit, Delete</span></span>
- <span data-ttu-id="8b89e-550">Funkce generování uživatelského rozhraní ASP.NET MVC pro zobrazení vlastností v tabulku HTML</span><span class="sxs-lookup"><span data-stu-id="8b89e-550">ASP.NET MVC's scaffolding feature for displaying properties in an HTML table</span></span>
- <span data-ttu-id="8b89e-551">Vlastní pomocné rutiny HTML pro zlepšení uživatelského prostředí</span><span class="sxs-lookup"><span data-stu-id="8b89e-551">Custom HTML helpers to improve user experience</span></span>
- <span data-ttu-id="8b89e-552">Metody akce, které reagují na HTTP GET nebo POST protokolu HTTP volání</span><span class="sxs-lookup"><span data-stu-id="8b89e-552">Action methods that react to either HTTP-GET or HTTP-POST calls</span></span>
- <span data-ttu-id="8b89e-553">Šablona sdíleného editor pro podobné zobrazení šablony, jako je vytvoření a úprava</span><span class="sxs-lookup"><span data-stu-id="8b89e-553">A shared editor template for similar View templates like Create and Edit</span></span>
- <span data-ttu-id="8b89e-554">Rozevírací seznamy, jako jsou elementy Form</span><span class="sxs-lookup"><span data-stu-id="8b89e-554">Form elements like drop-downs</span></span>
- <span data-ttu-id="8b89e-555">Anotací dat při ověření modelu</span><span class="sxs-lookup"><span data-stu-id="8b89e-555">Data annotations for Model validation</span></span>
- <span data-ttu-id="8b89e-556">Ověřování na straně klienta pomocí Nerušivého knihovny jQuery</span><span class="sxs-lookup"><span data-stu-id="8b89e-556">Client Side Validation using jQuery Unobtrusive library</span></span>

<a id="AppendixA"></a>

<a id="Appendix_A_Installing_Visual_Studio_Express_2012_for_Web"></a>
## <a name="appendix-a-installing-visual-studio-express-2012-for-web"></a><span data-ttu-id="8b89e-557">Příloha A: Instalace sady Visual Studio Express 2012 pro Web</span><span class="sxs-lookup"><span data-stu-id="8b89e-557">Appendix A: Installing Visual Studio Express 2012 for Web</span></span>

<span data-ttu-id="8b89e-558">Můžete nainstalovat **Microsoft Visual Studio Express 2012 pro Web** nebo jiném &quot;Express&quot; verzí pomocí **[instalačního programu webové platformy Microsoft](https://www.microsoft.com/web/downloads/platform.aspx)**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-558">You can install **Microsoft Visual Studio Express 2012 for Web** or another &quot;Express&quot; version using the **[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span></span> <span data-ttu-id="8b89e-559">Postupujte podle následujících pokynů vás provede kroky potřebné k instalaci *Visual studio Express 2012 pro Web* pomocí *instalačního programu webové platformy Microsoft*.</span><span class="sxs-lookup"><span data-stu-id="8b89e-559">The following instructions guide you through the steps required to install *Visual studio Express 2012 for Web* using *Microsoft Web Platform Installer*.</span></span>

1. <span data-ttu-id="8b89e-560">Přejděte na [ [ https://go.microsoft.com/?linkid=9810169 ](https://go.microsoft.com/?linkid=9810169) ](https://go.microsoft.com/?linkid=9810169).</span><span class="sxs-lookup"><span data-stu-id="8b89e-560">Go to [[https://go.microsoft.com/?linkid=9810169](https://go.microsoft.com/?linkid=9810169)](https://go.microsoft.com/?linkid=9810169).</span></span> <span data-ttu-id="8b89e-561">Případně, pokud jste již nainstalovali instalačního programu webové platformy, můžete otevřít a vyhledejte produkt &quot; <em>Visual Studio Express 2012 pro Web se sadou Windows Azure SDK</em>&quot;.</span><span class="sxs-lookup"><span data-stu-id="8b89e-561">Alternatively, if you already have installed Web Platform Installer, you can open it and search for the product &quot;<em>Visual Studio Express 2012 for Web with Windows Azure SDK</em>&quot;.</span></span>
2. <span data-ttu-id="8b89e-562">Klikněte na **nainstalovat**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-562">Click on **Install Now**.</span></span> <span data-ttu-id="8b89e-563">Pokud nemáte **instalačního programu webové platformy** budete přesměrováni na stáhněte a nainstalujte ji jako první.</span><span class="sxs-lookup"><span data-stu-id="8b89e-563">If you do not have **Web Platform Installer** you will be redirected to download and install it first.</span></span>
3. <span data-ttu-id="8b89e-564">Jednou **instalačního programu webové platformy** je otevřený, klikněte na tlačítko **nainstalovat** spustit instalační program.</span><span class="sxs-lookup"><span data-stu-id="8b89e-564">Once **Web Platform Installer** is open, click **Install** to start the setup.</span></span>

    <span data-ttu-id="8b89e-565">![Instalace sady Visual Studio Express](aspnet-mvc-4-helpers-forms-and-validation/_static/image22.png "instalace sady Visual Studio Express")</span><span class="sxs-lookup"><span data-stu-id="8b89e-565">![Install Visual Studio Express](aspnet-mvc-4-helpers-forms-and-validation/_static/image22.png "Install Visual Studio Express")</span></span>

    <span data-ttu-id="8b89e-566">*Instalace sady Visual Studio Express*</span><span class="sxs-lookup"><span data-stu-id="8b89e-566">*Install Visual Studio Express*</span></span>
4. <span data-ttu-id="8b89e-567">Čtení všech produktů licence a podmínky a klikněte na tlačítko **souhlasím** pokračujte.</span><span class="sxs-lookup"><span data-stu-id="8b89e-567">Read all the products' licenses and terms and click **I Accept** to continue.</span></span>

    ![Přijetí podmínek licence](aspnet-mvc-4-helpers-forms-and-validation/_static/image23.png)

    <span data-ttu-id="8b89e-569">*Přijetí podmínek licence*</span><span class="sxs-lookup"><span data-stu-id="8b89e-569">*Accepting the license terms*</span></span>
5. <span data-ttu-id="8b89e-570">Počkejte na dokončení procesu stahování a instalaci.</span><span class="sxs-lookup"><span data-stu-id="8b89e-570">Wait until the downloading and installation process completes.</span></span>

    ![Průběh instalace](aspnet-mvc-4-helpers-forms-and-validation/_static/image24.png)

    <span data-ttu-id="8b89e-572">*Průběh instalace*</span><span class="sxs-lookup"><span data-stu-id="8b89e-572">*Installation progress*</span></span>
6. <span data-ttu-id="8b89e-573">Až instalace skončí, klikněte na tlačítko **Dokončit**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-573">When the installation completes, click **Finish**.</span></span>

    ![Instalace byla dokončena.](aspnet-mvc-4-helpers-forms-and-validation/_static/image25.png)

    <span data-ttu-id="8b89e-575">*Instalace byla dokončena.*</span><span class="sxs-lookup"><span data-stu-id="8b89e-575">*Installation completed*</span></span>
7. <span data-ttu-id="8b89e-576">Klikněte na tlačítko **ukončovací** zavřete instalačního programu webové platformy.</span><span class="sxs-lookup"><span data-stu-id="8b89e-576">Click **Exit** to close Web Platform Installer.</span></span>
8. <span data-ttu-id="8b89e-577">Chcete-li spustit nástroj Visual Studio Express for Web, přejděte **Start** obrazovky a začít psát &quot; **VS Express**&quot;, klikněte na **VS Express for Web** dlaždice.</span><span class="sxs-lookup"><span data-stu-id="8b89e-577">To open Visual Studio Express for Web, go to the **Start** screen and start writing &quot;**VS Express**&quot;, then click on the **VS Express for Web** tile.</span></span>

    ![VS Express for Web dlaždice](aspnet-mvc-4-helpers-forms-and-validation/_static/image26.png)

    <span data-ttu-id="8b89e-579">*VS Express for Web dlaždice*</span><span class="sxs-lookup"><span data-stu-id="8b89e-579">*VS Express for Web tile*</span></span>

<a id="AppendixB"></a>

<a id="Appendix_B_Using_Code_Snippets"></a>
## <a name="appendix-b-using-code-snippets"></a><span data-ttu-id="8b89e-580">Příloha B: Používání fragmentů kódu</span><span class="sxs-lookup"><span data-stu-id="8b89e-580">Appendix B: Using Code Snippets</span></span>

<span data-ttu-id="8b89e-581">Pomocí fragmentů kódu máte všechny kód, který je třeba na dosah ruky.</span><span class="sxs-lookup"><span data-stu-id="8b89e-581">With code snippets, you have all the code you need at your fingertips.</span></span> <span data-ttu-id="8b89e-582">Testovací prostředí dokumentu zjistíte přesně kdy je můžete využít, jak je znázorněno na následujícím obrázku.</span><span class="sxs-lookup"><span data-stu-id="8b89e-582">The lab document will tell you exactly when you can use them, as shown in the following figure.</span></span>

<span data-ttu-id="8b89e-583">![Používání fragmentů kódu sady Visual Studio pro vložení kódu do projektu](aspnet-mvc-4-helpers-forms-and-validation/_static/image27.png "pomocí sady Visual Studio fragmenty kódu pro vložení kódu do projektu")</span><span class="sxs-lookup"><span data-stu-id="8b89e-583">![Using Visual Studio code snippets to insert code into your project](aspnet-mvc-4-helpers-forms-and-validation/_static/image27.png "Using Visual Studio code snippets to insert code into your project")</span></span>

<span data-ttu-id="8b89e-584">*Používání fragmentů kódu sady Visual Studio pro vložení kódu do projektu*</span><span class="sxs-lookup"><span data-stu-id="8b89e-584">*Using Visual Studio code snippets to insert code into your project*</span></span>

<span data-ttu-id="8b89e-585">***Chcete-li přidat fragment kódu pomocí klávesnice (C# jenom)***</span><span class="sxs-lookup"><span data-stu-id="8b89e-585">***To add a code snippet using the keyboard (C# only)***</span></span>

1. <span data-ttu-id="8b89e-586">Umístěte kurzor, kam chcete vložit kód.</span><span class="sxs-lookup"><span data-stu-id="8b89e-586">Place the cursor where you would like to insert the code.</span></span>
2. <span data-ttu-id="8b89e-587">Začněte psát název fragmentu kódu (bez mezer nebo pomlčky).</span><span class="sxs-lookup"><span data-stu-id="8b89e-587">Start typing the snippet name (without spaces or hyphens).</span></span>
3. <span data-ttu-id="8b89e-588">Podívejte se na jako IntelliSense zobrazí odpovídající názvy fragmenty kódu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-588">Watch as IntelliSense displays matching snippets' names.</span></span>
4. <span data-ttu-id="8b89e-589">Vyberte správný fragment kódu (nebo pokračujte v psaní dokud nebude vybraný celý fragment název).</span><span class="sxs-lookup"><span data-stu-id="8b89e-589">Select the correct snippet (or keep typing until the entire snippet's name is selected).</span></span>
5. <span data-ttu-id="8b89e-590">Stiskněte klávesu Tabulátor dvakrát pro vložení fragmentu do umístění kurzoru.</span><span class="sxs-lookup"><span data-stu-id="8b89e-590">Press the Tab key twice to insert the snippet at the cursor location.</span></span>

<span data-ttu-id="8b89e-591">![Začněte psát název fragmentu kódu](aspnet-mvc-4-helpers-forms-and-validation/_static/image28.png "začněte psát název fragmentu kódu")</span><span class="sxs-lookup"><span data-stu-id="8b89e-591">![Start typing the snippet name](aspnet-mvc-4-helpers-forms-and-validation/_static/image28.png "Start typing the snippet name")</span></span>

<span data-ttu-id="8b89e-592">*Začněte psát název fragmentu kódu*</span><span class="sxs-lookup"><span data-stu-id="8b89e-592">*Start typing the snippet name*</span></span>

<span data-ttu-id="8b89e-593">![Stiskněte klávesu Tab k vybrání fragmentu zvýrazněné](aspnet-mvc-4-helpers-forms-and-validation/_static/image29.png "stisknutím klávesy Tab k výběru zvýrazněné fragment kódu")</span><span class="sxs-lookup"><span data-stu-id="8b89e-593">![Press Tab to select the highlighted snippet](aspnet-mvc-4-helpers-forms-and-validation/_static/image29.png "Press Tab to select the highlighted snippet")</span></span>

<span data-ttu-id="8b89e-594">*Stiskněte klávesu Tab k výběru zvýrazněné fragment kódu*</span><span class="sxs-lookup"><span data-stu-id="8b89e-594">*Press Tab to select the highlighted snippet*</span></span>

<span data-ttu-id="8b89e-595">![Stisknutím klávesy Tab znovu a fragment kódu se rozbalí](aspnet-mvc-4-helpers-forms-and-validation/_static/image30.png "znovu stisknutím klávesy Tab a fragment kódu se rozbalí.")</span><span class="sxs-lookup"><span data-stu-id="8b89e-595">![Press Tab again and the snippet will expand](aspnet-mvc-4-helpers-forms-and-validation/_static/image30.png "Press Tab again and the snippet will expand")</span></span>

<span data-ttu-id="8b89e-596">*Stisknutím klávesy Tab znovu a fragment kódu se rozbalí.*</span><span class="sxs-lookup"><span data-stu-id="8b89e-596">*Press Tab again and the snippet will expand*</span></span>

<span data-ttu-id="8b89e-597">***Chcete-li přidat fragment kódu pomocí myši (C#, Visual Basic a XML)*** 1.</span><span class="sxs-lookup"><span data-stu-id="8b89e-597">***To add a code snippet using the mouse (C#, Visual Basic and XML)*** 1.</span></span> <span data-ttu-id="8b89e-598">Klikněte pravým tlačítkem na místo, kam chcete vložit fragment kódu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-598">Right-click where you want to insert the code snippet.</span></span>

1. <span data-ttu-id="8b89e-599">Vyberte **Vložit fragment** následovaný **Moje fragmenty kódu**.</span><span class="sxs-lookup"><span data-stu-id="8b89e-599">Select **Insert Snippet** followed by **My Code Snippets**.</span></span>
2. <span data-ttu-id="8b89e-600">Kliknutím na vyberte relevantní fragment kódu ze seznamu.</span><span class="sxs-lookup"><span data-stu-id="8b89e-600">Pick the relevant snippet from the list, by clicking on it.</span></span>

<span data-ttu-id="8b89e-601">![Klikněte pravým tlačítkem, ve které chcete vložit fragment kódu a vyberte Vložit fragment](aspnet-mvc-4-helpers-forms-and-validation/_static/image31.png "klikněte pravým tlačítkem, ve které chcete vložit fragment kódu a vyberte Vložit fragment")</span><span class="sxs-lookup"><span data-stu-id="8b89e-601">![Right-click where you want to insert the code snippet and select Insert Snippet](aspnet-mvc-4-helpers-forms-and-validation/_static/image31.png "Right-click where you want to insert the code snippet and select Insert Snippet")</span></span>

<span data-ttu-id="8b89e-602">*Klikněte pravým tlačítkem na, ve které chcete vložit fragment kódu a vyberte Vložit fragment*</span><span class="sxs-lookup"><span data-stu-id="8b89e-602">*Right-click where you want to insert the code snippet and select Insert Snippet*</span></span>

<span data-ttu-id="8b89e-603">![Vyberte si relevantní fragment kódu ze seznamu, kliknutím na](aspnet-mvc-4-helpers-forms-and-validation/_static/image32.png "relevantní fragment kódu ze seznamu vyberte kliknutím na")</span><span class="sxs-lookup"><span data-stu-id="8b89e-603">![Pick the relevant snippet from the list, by clicking on it](aspnet-mvc-4-helpers-forms-and-validation/_static/image32.png "Pick the relevant snippet from the list, by clicking on it")</span></span>

<span data-ttu-id="8b89e-604">*Vyberte si relevantní fragment kódu ze seznamu, kliknutím na*</span><span class="sxs-lookup"><span data-stu-id="8b89e-604">*Pick the relevant snippet from the list, by clicking on it*</span></span>
