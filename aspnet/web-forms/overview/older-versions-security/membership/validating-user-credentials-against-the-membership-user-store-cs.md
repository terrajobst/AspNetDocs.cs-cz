...
title: Ověřují se přihlašovací údaje uživatele pro uživatelské úložiště členstvíC#() | Microsoft Docs
...
Tento kód je výjimečně jednoduchý. Začneme voláním metody `Membership.ValidateUser` s předáním zadaného uživatelského jména a hesla. Pokud tato metoda vrátí hodnotu true, pak je uživatel přihlášen k webu prostřednictvím metody RedirectFromLoginPage třídy `FormsAuthentication`. (Jak jsme probrali <a id="Tutorial2"> </a>v kurzu [*Přehled ověřování formulářů*](../introduction/an-overview-of-forms-authentication-cs.md) , `FormsAuthentication.RedirectFromLoginPage` vytvoří lístek ověřování formuláře a pak přesměruje uživatele na příslušnou stránku.) Pokud jsou přihlašovací údaje neplatné, zobrazí se ale popisek `InvalidCredentialsMessage`, který informuje uživatele o tom, že jeho uživatelské jméno nebo heslo nebylo správné.
...
[![komentovat existující deklarativní značky a zdrojový kód v Login. aspx](validating-user-credentials-against-the-membership-user-store-cs/_static/image5.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image4.png)
...
Vlastnost text v následujícím časovém poli pro zapamatování času se dá nastavit prostřednictvím [`RememberMeText property`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.login.remembermetext.aspx)ovládacího prvku přihlášení a její výchozí stav zaškrtnutí se dá nakonfigurovat přes [`RememberMeSet property`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.login.remembermeset.aspx) (což je výchozí hodnota false). Pokračujte a nastavte vlastnost `RememberMeSet` na hodnotu true, aby se ve výchozím nastavení kontrolovalo zaškrtávací políčko Zapamatovat mě Next Time.

Ovládací prvek Login nabízí dvě vlastnosti pro úpravu rozložení ovládacích prvků uživatelského rozhraní. [`TextLayout property`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.login.textlayout.aspx) určuje, zda jsou uživatelské jméno a heslo: popisky zobrazeny nalevo od jejich odpovídajících textových polí (výchozí nastavení), nebo nad nimi. [`Orientation property`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.login.orientation.aspx) určuje, zda jsou vstupy uživatelského jména a hesla umístěny svisle (jedna nad druhou) nebo horizontálně. Nechám tyto dvě vlastnosti nastavené na výchozí hodnoty, ale doporučujeme, abyste si vyzkoušeli nastavení těchto dvou vlastností na jiné než výchozí hodnoty, aby se zobrazil výsledný efekt.
...
[![hodnoty vlastností ovládacího prvku pro přihlašovací údaje určují jeho vzhled.](validating-user-credentials-against-the-membership-user-store-cs/_static/image14.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image13.png)
...
[![převedení přihlašovacího ovládacího prvku na šablonu](validating-user-credentials-against-the-membership-user-store-cs/_static/image17.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image16.png)
...
[![méně vlastností, které jsou k dispozici, když je ovládací prvek přihlášení převeden na šablonu](validating-user-credentials-against-the-membership-user-store-cs/_static/image20.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image19.png)
...
[![přihlašovací řízení teď obsahuje textové pole pro e-mailovou adresu uživatele.](validating-user-credentials-against-the-membership-user-store-cs/_static/image23.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image22.png)
...
[![pracovního postupu ověřování ovládacího prvku přihlášení](validating-user-credentials-against-the-membership-user-store-cs/_static/image26.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image25.png)
...
[![tito se nemůže přihlásit při zadání nesprávné e-mailové adresy.](validating-user-credentials-against-the-membership-user-store-cs/_static/image29.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image28.png)
...
[![tito provedlo příliš mnoho neplatných pokusů o přihlášení a byl uzamčen.](validating-user-credentials-against-the-membership-user-store-cs/_static/image32.png)](validating-user-credentials-against-the-membership-user-store-cs/_static/image31.png)
