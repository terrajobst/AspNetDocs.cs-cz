...
## <a name="additional"></a>Další
  * Upřesněte cifES a nieES algoritmy. Zavře #1826

## <a name="build"></a>Sestavení
  * Zahrnout Minifikovaný verze balíčku NPM
  * Údaj dev závislosti na nejnovější verze

## <a name="core"></a>Jádro
  * Přidáte vazbu pro vstup typu tlačítka. Zavře #1891
  * Podpora jquery3. Zavře #1866
  * Změna jQuery alias ' výraz [":"] "na"expr.pseudos"

## <a name="localisation"></a>Lokalizaci
  * Přidání urdštiny překladu. Zavře #1873.

## <a name="localization"></a>Lokalizace
  * Oprava nesprávného – přípona souboru pro az překlad. Zavře #1890.
  * Přidání chybějící překlad v pt-BR (zavře 1897 #)
  * Oprava překlep v souboru jazyka arabien.

## <a name="tests"></a>Testy
  * Upgradujte QUnit 2.0.

## <a name="umd"></a>UMD
  * Lepší podpora CommonJS.

<a name="1151--2016-07-22"></a>1.15.1 / 2016-07-22
==================

## <a name="additional"></a>Další
  * Vyřešit více ověřování typ mime
  * IBAN vyžadují alespoň 5 znaků (#1797 zavře, řeší 1674 #)
  * Odkaz na správnou notEqualTo jQuery

## <a name="core"></a>Jádro
  * Neúspěšný test pro #1805 přidán
  * Oprava ověření skupiny s 3 a více polí
  * Oprava regrese zavedený #1644 a 1657 # (zavře #1800)
  * Aktualizovat ověřovací krok zpracování s plovoucí desetinnou čárkou body správně
  * Opravte chybu při volání metody $.fn.rules() v prvku bez formuláře
  * Volání `errorPlacement` v oboru program pro ověření
  * Oprava potíží s contenteditable prvků ve formulářích, kde události pro jedno vstupní ověření by způsobilo výjimky

## <a name="demo"></a>Ukázka
  * Přidat odkazy na ukázky index.html Bootstrap a Semantic uživatelského rozhraní
  * Použití `.on()` místo `.validateDelegate()`

## <a name="localization"></a>Lokalizace
  * Přidané jazykově Azeri

## <a name="tests"></a>Testy
  * Přidání regresní testy jednotek pro žádosti o přijetí změn č. 1760

<a name="1150--2016-02-24"></a>1.15.0 / 2016-02-24
==================

## <a name="all"></a>Všechny
  * Problémy stylu Opravený kód

## <a name="core"></a>Jádro
  * `resetForm` mělo odstranit také `valid` třídy z prvků.
  * Pokud už zvýrazněný při používání vzdálené pravidlo unhighlighting pole.
  * Vytvoření vazby `blur` událost pouze jednou v `equalTo` pravidlo
  * Oprava chyby při volání .rules() na prázdný jquery nastavit.
  * Oprava zpracování chybové zprávy s vstupní skupiny.
  * Oprava TypeError v `showLabel` při použití `groups` nastavení
  * Přidání způsob, jak předat název metody vzdálené
  * Ověření nepodaří aktivovat, když se další pole se vyplní již (opravy č. 1508)
  * Požadované pravidlo přednost nad číslo & číslic pravidla
  * Odebrat chyby není skrytý ale třída vstupních chyb
  * Vzdálené ověření používá nesprávný chybové zprávy
  * Pole pevné zvýraznění vzdálené ověření.
  * Oprava `:filled` selektor pro více prvků vyberete.
  * Přidání dokumentu odkaz na jQuery.validator.methods
  * Přesuňte zpracování z zpráv `formatAndAdd` do `defaultMessage`
  * ErrorList by měl obsahovat pouze chyby, které by měl
  * Extrahovat název souboru bez zahrnutí "C:\fakepath\"
  * Podpora atribut krok HTML5. Opravy #1295
  * Přidání podpory pro "až" třídu v odchozích požadavků
  * Přidání normalizátor (#1602)
  * Oddělit `creditcard` – metoda
  * Řídicí ID chyby pro použití v regulárního výrazu, ne k vytváření aria-describedby
  * Řídicí jednoduchých uvozovek v názvech vyhnout ještě působivější je vyvolána chyba
  * Místo hodnoty ověření pole pro přeskočení volání rozhraní api pomocí serializovaná data objektu žádosti
  * Přidání podpory pro contentEditable značky

## <a name="additional"></a>Další
  * BIC: Povolit číslice 1-9 na druhém místě umístění
  * Přijměte metoda regulární výraz by měl být uvozena správně.
  * Malá a velká písmena vyhledat BIC
  * Správné postalCodeCA vyloučit neplatná kombinace
  * Zkontrolujte mírnější postalCodeCA – metoda

## <a name="localization"></a>Lokalizace
  * Přidání Makedonský lokalizace.
  * Přidání zprávy chybí vzorek v Polština (adamwojtkiewicz)
  * Oprava perský překlad min/max zprávy.
  * Aktualizované messages_sk.js
  * Aktualizovat Malay překladu
  * Součástí zprávy z dalších metod
  * Zlepšení pt_BR překladu a opravě překlep v klíči "cifES".
