...
Další informace o provádění dotazů, které definujete v kódu, naleznete v tématu [jak: Přímé spuštění příkazů na zdroji dat](https://msdn.microsoft.com/library/ee358769.aspx).
...
V **vlastnosti připojení** dialogové okno pole, vyberte místní instanci systému SQL Server Express a název databáze `AlumniAssociation`.
...
Open *Alumni.aspx.cs*. Přidat `using` přístup k příkazu pro data, vrstvy a obslužné rutiny pro vnější `GridView` ovládacího prvku `RowDataBound` události:
...
(Poznámka: Tato stránka je zahrnutý v projektu ke stažení, ale aby to fungovalo, je nutné vytvořit databáze v místní instanci systému SQL Server Express; databáze není zahrnut jako *.mdf* soubor *aplikace\_Data* složky.)
...
- [Návod: Šablona POCO pro Entity Framework](https://blogs.msdn.com/b/adonet/archive/2010/01/25/walkthrough-poco-template-for-the-entity-framework.aspx) je to blogový příspěvek od týmu vývoje rozhraní Entity Framework, s odkazy na další blogové příspěvky o POCOs.
