...
| CheckBoxField | Zobrazuje zaškrtávací políčko pro každou položku v ovládacím prvku GridView. Tento typ pole sloupce běžně slouží k zobrazení polí s logickou hodnotou. |
...
| RowUpdated | Vyvolá se při kliknutí na tlačítko Aktualizovat řádek, ale po dokončení aktualizace řádku prvku GridView. Tato událost se často používá ke kontrole výsledky operace aktualizace. |
...
| EmptyDataTemplate | Definuje obsah prázdném datovém řádku zobrazí, když ovládacího prvku FormView je vázán na zdroj dat, který neobsahuje žádné záznamy. Tato šablona obvykle obsahuje obsah tak, aby upozornila uživatele, že zdroj dat neobsahuje žádné záznamy. |
...
| ItemTemplate | Definuje obsah pro řádek dat, když ovládacího prvku FormView je v režimu jen pro čtení. Tato šablona obvykle obsahuje obsah k zobrazení hodnot z existujícího záznamu. |
| InsertItemTemplate | Definuje obsah pro řádek dat, když ovládacího prvku FormView je v režimu vkládání. Tato šablona obvykle obsahuje vstupní ovládací prvky a příkazového tlačítka, pomocí kterého může uživatel přidat nový záznam. |
...
| Aktualizace | "Update" | Umožňuje při aktualizaci operace pokus o aktualizaci zobrazený záznam ve zdroji dat hodnotami, které zadal uživatel. Vyvolává události ItemUpdating a ItemUpdated. |
...
| CheckBoxField | Zaškrtávací políčko se zobrazí v ovládacím prvku DetailsView. Tento typ pole řádku běžně slouží k zobrazení polí s logickou hodnotou. |
...
Ovládací prvek DetailsView poskytuje několik událostí, které můžete programovat proti. To umožňuje spustit vlastní rutiny pokaždé, když dojde k události. Následující tabulka uvádí události podporována ovládacím prvku DetailsView. Tyto události ovládacího prvku DetailsView také dědí z jejích základních tříd: Vázání dat, datové vazby, odstraněn, Init, zatížení, PreRender a vykreslování.
...
| --- | --- |
| Žádná | Zaškrtávací políčka nejsou zobrazeny na všechny uzly. Toto je výchozí nastavení. |
...
## <a name="lab-1-configuring-an-aspnet-menu-control"></a>Lab 1: Konfigurace ovládacího prvku Menu technologie ASP.NET
...
## <a name="lab-2-dynamically-binding-a-treeview-control"></a>Lab 2: Dynamické vazby prvku TreeView
...
## <a name="lab-3-details-view-and-editing-data-using-a-gridview-and-detailsview"></a>Lab 3: Zobrazení podrobností a upravování dat pomocí ovládacího prvku GridView a prvku DetailsView.
